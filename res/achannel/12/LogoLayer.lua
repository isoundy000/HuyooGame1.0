...
function LogoLayer:onCreate()
    ...
    self:runAction(cc.Sequence:create(cc.DelayTime:create(0.2),cc.CallFunc:create(function(sender,event) 
        local logo = ccui.ImageView:create(string.format("channel/%d/logo.png",CHANNEL_ID))
        self:addChild(logo)
        ...
        cc.SimpleAudioEngine:getInstance():playEffect(string.format("channel/%d/logo.mp3",CHANNEL_ID))
    end)))
end
