...
--@param    type: 0文本提示   1确定取消  2确定  3同意拒绝  4 复制微信
...
--require("common.MsgBoxLayer"):create(4,nil,"请稍后...",okCallback)
...
function MsgBoxLayer:onCreate(type,node,...)
    ...
    local csb = cc.CSLoader:createNode("MsgBoxLayer.csb")
    if type == 5 then
        csb = cc.CSLoader:createNode("NewMallLayer.csb") 
    end 
    self:addChild(csb)
    ...
    if type ~= 5 then
        ...
        uiPanel_msgcopy = ccui.Helper:seekWidgetByName(self.root,"Panel_msgcopy")
    end 
    if type == 0 then
        --文本提示
        ...
        uiPanel_msgcopy:removeFromParent()
...
--        Common:playPopupAnim(uiPanel_agreeCancel)
    elseif type == 4 then
          ...
          uiPanel_agreeCancel:removeFromParent()
          uiPanel_tips:removeFromParent()
          uiPanel_msgcopy:setVisible(true)
          local uiText_contents = ccui.Helper:seekWidgetByName(self.root,"Text_contents")
          uiText_contents:setString(params[1]) 
          local uiButton_ok = ccui.Helper:seekWidgetByName(self.root,"Button_ok")
          Common:addTouchEventListener(uiButton_ok,function()
