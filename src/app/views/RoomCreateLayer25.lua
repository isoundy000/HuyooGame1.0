...
function RoomCreateLayer:onCreate(parameter)
    ...
    if self.recordCreateParameter["bPlayerCount"] ~= nil and self.recordCreateParameter["bPlayerCount"] == 2 then
        ...
        if self.wKindID == 25 and CHANNEL_ID ~= 20 and  CHANNEL_ID ~= 21 then
            ...
            if self.recordCreateParameter["bStartCard"] ~= nil and self.recordCreateParameter["bStartCard"] == 3 then
                items[1]:setBright(true)
            else
                items[1]:setBright(false)
            end
    ...
    Common:addCheckTouchEventListener({items[6]},true, function(index)   
        if items[6]:isBright() then
            items[9]:setBright(false)
            items[9]:setEnabled(false)
            items[9]:setVisible(false)
            items[9]:setColor(cc.c3b(170,170,170))
        else
            local itemsRS = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(1),"ListView_parameter"):getItems()   
            if itemsRS[1]:isBright() then       
