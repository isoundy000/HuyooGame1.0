...
local PerfectInfoLayer_6 = class("PerfectInfoLayer_6", function()
    return ccui.Layout:create()
end)

local PerfectInfoLayer_6 = class("PerfectInfoLayer_6", cc.load("mvc").ViewBase)

function PerfectInfoLayer_6:onEnter()
...
function PerfectInfoLayer_6:onExit()
...
function PerfectInfoLayer_6:onCleanup()

end

function PerfectInfoLayer_6:onCreate(expressCallback, quickCallback)    
    self:initUI()
end

-- ----------------------------------------------------------------UI--------------------------------------------------------------

function PerfectInfoLayer_6:initUI()

    local visibleSize = cc.Director:getInstance():getVisibleSize()
    local csb = cc.CSLoader:createNode("PerfectInfoLayer_6.csb")
    self:addChild(csb)
...
function PerfectInfoLayer_6:OkInfo()
    --获取所有输入信息
    self:GetAllInfo()
...
function PerfectInfoLayer_6:Showdata()
...
function PerfectInfoLayer_6:INFO_SET_USER_DETAIL(event)--INFO_SET_USER_DETAIL
...
function PerfectInfoLayer_6:UPDATE_SELF_USER_DETAIL(event)--INFO_SET_USER_DETAIL
    local data = event._usedata
end

function PerfectInfoLayer_6:SUB_CL_TASK_REWARD( event )
    local data = event._usedata
    if  data  then       -- ~= ""
      if data["ret"] == 0  then 
...
function PerfectInfoLayer_6:GetAllInfo()
    --姓名
    local uiTextField_name = ccui.Helper:seekWidgetByName(self.root,"TextField_name")
...
function PerfectInfoLayer_6:TestShenfz(shenfz)   
    --检验身份证  
...
function PerfectInfoLayer_6:SUB_CL_SET_USER_DETAIL( event )
    local data = event._usedata
