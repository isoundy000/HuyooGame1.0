...
local GuildActivityLayer = class("GuildActivityLayer1", cc.load("mvc").ViewBase)
...
function GuildActivityLayer:onCreate()
    ...
    local csb = cc.CSLoader:createNode("GuildActivityLayer1.csb")
    self:addChild(csb)
    ...
    self.uiPanel_show:setVisible(false)
    self.uiPanel_action:setVisible(true)
    
    Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root,"Button_enter"),function() 
        self.uiPanel_show:setVisible(false)
        self.uiPanel_action:setVisible(true)
    end)
    Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root,"Button_look"),function() 
    ...
    UserData.GuildActivity.RequestRanking1()
   -- self:Action()  
end
...
function GuildActivityLayer:SUB_SC_JSONCALLACTIVI(event)
    ...
    if data["ret"] ~= 0 then    
        ...
        local Text_mynum1 = ccui.Helper:seekWidgetByName(self.root,"Text_mynum1")
        Text_mynum1:setString("暂无数据")
        Text_mynum1:setTextColor(cc.c3b(22,0,0))    
        for i = 1 , 10 do
            ...
            item:setScale(0,1)
            item:runAction(cc.Sequence:create(cc.DelayTime:create(i*0.001),cc.ScaleTo:create(0.01,1.0,1.0)))  
        end   
    	return    	
    end 
...
--        local var = data["Msg"]         --pairs()
        ...
        print("游戏啊啊啊：",topData.RowNumber,topData.WinCount,topData.UserID,topData.NickName,pairs(data["Msg"]))
        local item = self.Panel_table:clone()
        item:setVisible(true)
        self.uiListView_table:pushBackCustomItem(item)   
        local uiImage_table = ccui.Helper:seekWidgetByName(item,"Image_table")
        ...
        if  topData.UserID == UserData.User.userID then 
            ...
            Text_mynum:setTextColor(cc.c3b(22,0,0))   
            local Text_mynum1 = ccui.Helper:seekWidgetByName(self.root,"Text_mynum1")
            Text_mynum1:setString("第"..tostring(topData.RowNumber).."名")
            Text_mynum1:setTextColor(cc.c3b(22,0,0))          
            local Text_myname = ccui.Helper:seekWidgetByName(self.root,"Text_myname")
