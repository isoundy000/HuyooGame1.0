...
local GuilLayer = class("GuilLayer", cc.load("mvc").ViewBase)

function GuilLayer:onEnter()
    ...
    EventMgr:registListener(EventType.RET_UPDATE_GUILD,self,self.RET_UPDATE_GUILD)    --更改公会公告 
...
function GuilLayer:onExit()
    ...
    EventMgr:unregistListener(EventType.RET_UPDATE_GUILD,self,self.RET_UPDATE_GUILD)    --更改公会公告 
...
function GuilLayer:onCreate()
    ...
    local csb = cc.CSLoader:createNode("GuildLayer.csb")
    self:addChild(csb)
...
function GuilLayer:showUI(index)
...
function GuilLayer:updateGuilInfo()
    if UserData.Guild.dwGuildID == 0 then
        return
    end
...
--    Text_num:setString(UserData.Guil.societyMenberCount)
    ...
    local uiButton_setting = ccui.Helper:seekWidgetByName(self.root,"Button_setting")
    local uiButton_modify = ccui.Helper:seekWidgetByName(self.root,"Button_modify")    
    if UserData.Guild.dwPresidentID == UserData.User.userID then
        uiButton_setting:setVisible(true)
        uiButton_modify:setVisible(true)
    else
        uiButton_setting:setVisible(false)
        uiButton_modify:setVisible(false)
    end
end

function GuilLayer:initFirst()
    local roomNumber = {}  
    local roomNum = ""
...
function GuilLayer:initGuilInfo()
    if UserData.Guild.dwGuildID == 0 then
        return
    end
    local uiButton_share = ccui.Helper:seekWidgetByName(self.root,"Button_share")
    ...
    Common:addTouchEventListener(ccui.Helper:seekWidgetByName(self.root,"Button_modify"),function() 
        print("跳后台")
        self.Panel_op = ccui.Helper:seekWidgetByName(self.root,"Panel_op") 
        self.Panel_op:addChild(require("app.MyApp"):create():createView("ModifyClubBulletinLayer"))


    end)    
--    local uiListView_proxyRecord = ccui.Helper:seekWidgetByName(self.root,"ListView_proxyRecord")
--    self.uiPanel_proxyInfo = uiListView_proxyRecord:getItem(0)
--    self.uiPanel_proxyInfo:retain()
--    uiListView_proxyRecord:removeAllItems()
    self:updateGuilInfo()
end
    


function GuilLayer:SUB_SC_ACTIONRESULT(event)
...
function GuilLayer:RET_GET_GUILD_INFO_BY_GUILDID(event)
    local data = event._usedata
...
function GuilLayer:RET_JOIN_GUILD(event)
    local data = event._usedata   
    if data.ret == 0 then   
...
function GuilLayer:RET_SETTINGS_GUILD(event)
    local data = event._usedata
    if data == nil then
...
function GuilLayer:EVENT_TYPE_TO_VIEW_GUILD(event)
    self:initFirst()
    self:initGuilInfo()
    self:updateGuilInfo()
    if CHANNEL_ID == 4 or CHANNEL_ID == 5 then
...
function GuilLayer:RET_UPDATE_GUILD()
    self:updateGuilInfo()
end

function GuilLayer:RET_SC_SUB_GET_PROXY_RECORD(event)
--    local data = event._usedata
--    local uiListView_proxyRecord = ccui.Helper:seekWidgetByName(self.root,"ListView_proxyRecord")
--    local item = self.uiPanel_proxyInfo:clone()
--    uiListView_proxyRecord:pushBackCustomItem(item)
--    local uiText_time = ccui.Helper:seekWidgetByName(item,"Text_time")
--    uiText_time:setTextColor(cc.c3b(15,9,8))
--    local date = os.date("*t",data.dwOperTime)
--    uiText_time:setString(string.format("%d-%02d-%02d %02d:%02d:%02d",date.year,date.month,date.day,date.hour,date.min,date.sec))
--    local uiText_game = ccui.Helper:seekWidgetByName(item,"Text_game")
--    uiText_game:setTextColor(cc.c3b(15,9,8))
--    if StaticData.Games[data.wKindID] ~= nil then
--        uiText_game:setString(StaticData.Games[data.wKindID].name)   
--    else
--        uiText_game:setString("")
--    end
--    local uiText_gameCount = ccui.Helper:seekWidgetByName(item,"Text_gameCount")
--    uiText_gameCount:setTextColor(cc.c3b(15,9,8))
--    uiText_gameCount:setString(string.format("%d局",data.wGameCount))
