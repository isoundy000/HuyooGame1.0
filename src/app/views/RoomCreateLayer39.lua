...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer39.csb")
    self:addChild(csb)
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["FangXing"] ~= nil and self.recordCreateParameter["FangXing"]== 0 then 
        items[2]:setBright(true)
    else
        items[1]:setBright(true)
    end
    ...
    if self.recordCreateParameter["bCanHuXi"] ~= nil and self.recordCreateParameter["bCanHuXi"] ~= 10 then
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(2),"ListView_parameter"):getItems()
    if items[1]:isBright() then
    	tableParameter.FangXing = 1
    elseif items[2]:isBright() then
        tableParameter.FangXing = 0
    else
        return
    end
    if tableParameter.FangXing == 1 then     
    ...
    tableParameter.bCanSiShou = 0
    tableParameter.bCanJuShouZuoSheng = 0
