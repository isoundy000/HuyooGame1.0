...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer52.csb")
    self:addChild(csb)
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["bMaCount"] ~= nil and self.recordCreateParameter["bMaCount"] == 6 then
        items[3]:setBright(true)
    elseif self.recordCreateParameter["bMaCount"] ~= nil and self.recordCreateParameter["bMaCount"] == 4 then
        items[2]:setBright(true)
    elseif self.recordCreateParameter["bMaCount"] ~= nil and self.recordCreateParameter["bMaCount"] == 0 then
    ...
    --可胡七小对
    if self.recordCreateParameter["bHuQD"] ~= nil and self.recordCreateParameter["bHuQD"] == 1 then
        items[1]:setBright(true)
    else
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    --可胡七小对
    if items[1]:isBright() then
        tableParameter.bHuQD = 1
    else
        tableParameter.bHuQD = 0
    end
    --可点炮
