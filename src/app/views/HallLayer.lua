...
function HallLayer:onCreate(parames)
    ...
    local csb = cc.CSLoader:createNode(halllayer)
    self:addChild(csb)
    ...
    if uiButton_roomCardBg ~= nil then   
        ...
        Common:addTouchEventListener(uiButton_roomCardBg,function()             
            if CHANNEL_ID ~= 10 and  CHANNEL_ID ~= 11  then 
                ...
                require("common.MsgBoxLayer"):create(4,nil,"客服微信:tykf668","tykf668") 
            end 
        end)    
...
--        end 
    ...
    if uiButton_record ~= nil then
        Common:addTouchEventListener(uiButton_record,function() 
            ...
            -- local params = {
            --     frontPath = 'puke/card/cardcuopai/puke_0_7',
            --     backPath = 'puke/card/cardcuopai/puke_bg0',
            --     childPath = 'puke/card/cardcuopai/5_black',
            --     childColor = 'puke/card/cardcuopai/0_Small',
            --     pos = cc.p(500,500),
                
            -- }
            -- local box = require("app.views.RubbingCards").new(params)
    ...
    -- if CHANNEL_ID == 4 or  CHANNEL_ID == 5  then 
    --    uiButton_club:setVisible(false)
    -- end
...
function HallLayer:RET_SPORTS_STATE(event)
    ...
    if data.isOpenSports == true then
        ...
        if CHANNEL_ID ~= 20 and CHANNEL_ID ~= 21 then
            uiButton_sports:setVisible(true)
