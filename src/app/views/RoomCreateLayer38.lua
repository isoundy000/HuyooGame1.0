...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer38.csb")
    self:addChild(csb)
    ...
    elseif self.recordCreateParameter["FanXing"] ~= nil and self.recordCreateParameter["FanXing"]["bAddTun"] == 1 then    
        items[1]:setBright(true)
    elseif self.recordCreateParameter["FanXing"] ~= nil and self.recordCreateParameter["FanXing"]["bAddTun"] == 3 then
        items[2]:setBright(true)
    else
        for key, var in pairs(items) do
            var:setBright(false)
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    if tableParameter.FanXing.bType ~= 0 then
        ...
        elseif items[2]:isBright() then
            tableParameter.FanXing.bAddTun = 3
        else
            return
        end
    else
