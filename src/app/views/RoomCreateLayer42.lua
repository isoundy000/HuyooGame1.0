...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer42.csb")
    self:addChild(csb)
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["bZhaNiao"] ~= nil and self.recordCreateParameter["bZhaNiao"] == 0 then
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["bZhaNiao"] ~= nil and self.recordCreateParameter["m_bGameScore"] == 2 then
        items[2]:setBright(true)
    elseif self.recordCreateParameter["bZhaNiao"] ~= nil and self.recordCreateParameter["m_bGameScore"] == 4 then
    ...
    if self.recordCreateParameter["dwMingTang"] == nil or self.recordCreateParameter["m_bGameZiMo"] == 1 then
        items[1]:setBright(false)
    end
    if self.recordCreateParameter["dwMingTang"] == nil or self.recordCreateParameter["m_bGamePiaoHua"] == 0 then
        items[2]:setBright(true)
    end
    if self.recordCreateParameter["dwMingTang"] == nil or self.recordCreateParameter["m_bGameZhuangScore"] == 1 then
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    if items[1]:isBright() then
        tableParameter.mailiao = 1
    elseif items[2]:isBright() then
        tableParameter.mailiao = 0
    else
        return
    end
    --分数翻倍
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(2),"ListView_parameter"):getItems()
    if items[1]:isBright() then
        tableParameter.fanbei = 1
    elseif items[2]:isBright() then
        tableParameter.fanbei = 2
    elseif items[3]:isBright() then
        tableParameter.fanbei = 4
    else
        return
    end
    --选择玩法
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(3),"ListView_parameter"):getItems()
    if items[1]:isBright() then
        tableParameter.zimo = 1
    else
        tableParameter.zimo = 0
    end
    if items[2]:isBright() then
        tableParameter.piaohua = 1
    else
        tableParameter.piaohua = 0
    end
    if items[3]:isBright() then
        tableParameter.jiabei = 1
    else
        tableParameter.jiabei = 0
    end
    ...
    tableParameter.numpep = 4           -- 代表4人玩 （ 写死）,tableParameter.numpep
