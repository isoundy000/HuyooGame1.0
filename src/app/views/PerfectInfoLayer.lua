...
function PerfectInfoLayer:initUI()
    ...
    --电话号码
    local uiTextField_dianhua = ccui.Helper:seekWidgetByName(self.root,"TextField_dianhua")
     --验证码
    local uiTextField_yanzhenma = ccui.Helper:seekWidgetByName(self.root,"TextField_yanzhenma")
    uiTextField_yanzhenma:addTouchEventListener(function(sender,event)
    ...
    local uiPanel_channel = ccui.Helper:seekWidgetByName(self.root,"Panel_channel")
    local uiPanel_look = ccui.Helper:seekWidgetByName(self.root,"Panel_look")
    if  CHANNEL_ID == 20 or CHANNEL_ID == 21 then 
        uiPanel_channel:setVisible(false)
        uiPanel_look:setPositionY(uiPanel_look:getParent():getContentSize().height/2)
        
    end 

    print("玩家姓名",UserData.User.szRealName,UserData.User.zIDNumber,UserData.User.szPhone)
    if  UserData.User.szRealName ~= "" and UserData.User.zIDNumber ~= ""  and UserData.User.szPhone ~= ""  then       -- ~=and UserData.User.szEMail ~= "" ""
        self:Showdata()
        self.uiPanel_iocertification:setVisible(false)
        self.uiPanel_information:setVisible(true)
    end
    self.uiButton_Verification1 = ccui.Helper:seekWidgetByName(self.root,"Button_Verification1")
    self.uiButton_Verification2 = ccui.Helper:seekWidgetByName(self.root,"Button_Verification2")
    self.text_s = ccui.Helper:seekWidgetByName(self.root,"Text_time")
    self.text_s:setVisible(false)
    self.uiButton_Verification2:setBright(false)
    Common:addTouchEventListener(self.uiButton_Verification1,function()         
        self.uiButton_Verification1:setVisible(false)  
        self:Oktime()       
        if #uiTextField_dianhua:getString() == 11 then 
        self.Infodianhua = uiTextField_dianhua:getString()
        UserData.PerfectInfo:requestVerifyCode(uiTextField_dianhua:getString())
        else
            require("common.MsgBoxLayer"):create(0,nil,"手机号码输入错误")
        end
    end)
end
...
function PerfectInfoLayer:OkInfo()
    ...
    if  CHANNEL_ID ~= 20 and  CHANNEL_ID ~= 21 then 
        local is_Infodianhua =  self:isRightdianhua(self.Infoyanzhenma)
        if is_Infodianhua == false then
            require("common.MsgBoxLayer"):create(0,nil,"验证码输入错误")
            return
        elseif is_Infodianhua == true then
            print("手机号码输入正确")
        end
    else
    self.Infodianhua = 0
...
function PerfectInfoLayer:Showdata()
    ...
    uiText_name:setString(UserData.User.szRealName)
    local uiText_sfz = ccui.Helper:seekWidgetByName(self.root,"Text_sfz")
    uiText_sfz:setString(UserData.User.zIDNumber)
    local uiText_number = ccui.Helper:seekWidgetByName(self.root,"Text_number")
    uiText_number:setString(UserData.User.szPhone)
    if  CHANNEL_ID == 20 or CHANNEL_ID == 21 then 
        local uiText_3_0 = ccui.Helper:seekWidgetByName(self.root,"Text_3_0")
        uiText_3_0:setVisible(false)
        uiText_number:setVisible(false)
    end
end
...
function PerfectInfoLayer:GetAllInfo()
    ...
    --验证码
    local uiTextField_yanzhenma = ccui.Helper:seekWidgetByName(self.root,"TextField_yanzhenma")
    self.Infoyanzhenma = uiTextField_yanzhenma:getString()
end
