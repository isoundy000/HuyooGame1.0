...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer53.csb")
    self:addChild(csb)
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["bBettingType"] ~= nil and self.recordCreateParameter["bBettingType"] == 4 then
    ...
    if self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 5 then
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 3 then
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 2 then
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 0 then
        items[1]:setBright(true)
    ...
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 1 then
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 4  then
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 5 then
        items[1]:setBright(true)
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 3 then
        items[3]:setBright(true)
    elseif self.recordCreateParameter["bBankerType"] ~= nil and self.recordCreateParameter["bBankerType"] == 2 then
    ...
    --特殊选项
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(4),"ListView_parameter"):getItems()
    Common:addCheckTouchEventListener(items,true,function(index)end)
        -- if self.recordCreateParameter["bBankerType"] == nil or self.recordCreateParameter["bBankerType"] ~= 1 and self.recordCreateParameter["bBankerType"] ~= 5 then
        --     local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(4),"ListView_parameter"):getItems()
        --     local var = items[1]
        --     var:setColor(cc.c3b(170,170,170))
        --     var:setEnabled(false)
        ...
        --     items[1]:setBright(false)
        -- end
        if self.recordCreateParameter["bExtreme"] ~= nil and self.recordCreateParameter["bExtreme"] == 1 then 
            items[1]:setBright(true)
        else
            items[1]:setBright(false)
        end    
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    if items[1]:isBright() then
        tableParameter.bBettingType = 3
    elseif items[2]:isBright() then
        tableParameter.bBettingType = 4
    else
        return
    end
    --选择庄家
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(2),"ListView_parameter"):getItems()
    if items[1]:isBright() then
        tableParameter.bBankerType = 0
    elseif items[2]:isBright() then
        tableParameter.bBankerType = 1
    elseif items[3]:isBright() then
        tableParameter.bBankerType = 4
    end
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(3),"ListView_parameter"):getItems()
    if items[1]:isBright() then
        tableParameter.bBankerType = 5
    elseif items[2]:isBright() then
        tableParameter.bBankerType = 2
    elseif items[3]:isBright() then
        tableParameter.bBankerType = 3
    end
--    --特殊选项
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(4),"ListView_parameter"):getItems()
    -- if items[1]:isBright() then
    --     tableParameter.bPush = 1
    -- else
        tableParameter.bPush = 0
    -- end
    if items[1]:isBright() then
        tableParameter.bExtreme = 1
    else
        tableParameter.bExtreme = 0
    end

    if items[2]:isBright() then
        tableParameter.bCuopai = 1
    else
        tableParameter.bCuopai = 0
    end

    if items[3]:isBright() then
        tableParameter.bCanPlayingJoin = 1
    else
        tableParameter.bCanPlayingJoin = 0
    end   
...
--    --庄家倍数
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(5),"ListView_parameter"):getItems()
    if items[1]:isBright() then
        tableParameter.bMultiple = 1
      elseif items[2]:isBright() then
        tableParameter.bMultiple = 2
    elseif items[3]:isBright() then
        tableParameter.bMultiple = 3
    elseif items[4]:isBright() then
        tableParameter.bMultiple = 4
    else
        return
    end 
    tableParameter.bPlayerCount = 6
