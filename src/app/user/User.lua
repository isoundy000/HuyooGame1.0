...
function User:sendMsgConnectLogin(data)
    ...
    if PLATFORM_TYPE == cc.PLATFORM_OS_DEVELOPER or UserLevel <= 1 then
        local ret = NetMgr:getLoginInstance():connectGameSvr(SERVER_INFO.ip,SERVER_INFO.port)
        if ret == true then
        ...
        EventMgr:dispatch(EventType.EVENT_TYPE_CONNECT_LOGIN_FAILED,{SERVER_INFO.ip,SERVER_INFO.port})
    else
        local ret = NetMgr:getLoginInstance():connectGameSvr(string.format(StaticData.Condition[UserLevel].loginIp,SERVER_INFO.ip),SERVER_INFO.port)
        if ret == true then
            NetMgr:getLoginInstance():sendMsgToSvr(NetMsgId.MDM_GP_LOGON,NetMsgId.SUB_GP_LOGON_ACCOUNTS,"wwnsnsnsbdns",
                data.wKind,data.wType,64,data.szAccount,32,data.szNickName,256,
                data.szLogoInfo,data.cbGender,data.dwChannelID,64,data.szUnionid)
...
function User:sendMsgConnectLogic()
    local addr = self.logicData.szLogicServerURL
    if self.logicData.szLogicServerURL == "0" then
        addr = NetMgr:getLogicInstance().cppFunc:int2ip(self.logicData.dwServerAddr)
    end
    local UserLevel = self:readUserLevel()
    if PLATFORM_TYPE == cc.PLATFORM_OS_DEVELOPER or UserLevel <= 1 then
        local ret = NetMgr:getLogicInstance():connectGameSvr(addr,self.logicData.wServerPort)
        if ret == true then
        ...
        EventMgr:dispatch(EventType.EVENT_TYPE_CONNECT_LOGIC_FAILED,{addr,self.logicData.wServerPort})  
    else
        local ret = NetMgr:getLogicInstance():connectGameSvr(string.format(StaticData.Condition[UserLevel].logicIp,addr),self.logicData.wServerPort)
        if ret == true then
            NetMgr:getLogicInstance():sendMsgToSvr(NetMsgId.MDM_CL_LOGON,NetMsgId.REQ_CL_LOGON_USERID,"dns",self.logicData.dwUserID,64,self.logicData.szAuthCode)
            return
        end
