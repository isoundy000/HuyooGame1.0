...
function Game:saveCommonPlayways()
    ...
    local fp = io.open(FileName.talbeCommonPlayways,"wb+")
    fp:write(data)
    fp:close()
end
...
function Game:sendMsgConnectGame(data)  
    local addr = NetMgr:getGameInstance().cppFunc:int2ip(data.dwServerAddr)
    if Common:isDomain(data.szServerName) then
        addr = data.szServerName
    end
    local UserLevel = require("app.user.UserData").User:readUserLevel()
    if PLATFORM_TYPE == cc.PLATFORM_OS_DEVELOPER or UserLevel <= 1 then
        if NetMgr:getGameInstance():connectGameSvr(addr,data.wServerPort) == true then      
            NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_LOGON,NetMsgId.REQ_GR_LOGON_USERID,"daa",require("app.user.UserData").User.userID,LocationSystem.pos.x,LocationSystem.pos.y)
            self:addCommonGames(data.wKindID)
            return   
        end
        EventMgr:dispatch(EventType.EVENT_TYPE_CONNECT_GAME_FAILED,data)  
    else
        if NetMgr:getGameInstance():connectGameSvr(string.format(StaticData.Condition[UserLevel].gameIp,addr),data.wServerPort) == true then      
            NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_LOGON,NetMsgId.REQ_GR_LOGON_USERID,"daa",require("app.user.UserData").User.userID,LocationSystem.pos.x,LocationSystem.pos.y)
            self:addCommonGames(data.wKindID)
            return    
        end
