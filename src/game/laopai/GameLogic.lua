...
--        end
        for i = 1 , 34 do
            if (cbCardIndex[i] ~= 0) then   --and i~= 28 
                for j = 1 , cbCardIndex[i] do
                    assert(bPosition<34)
                    bPosition = bPosition + 1
                    cbCardData[bPosition]= self:SwitchToCardDataOne(i)
...
function GameLogic:SwitchToCardIndexTwo(cbCardData, cbCardCount)--麻将转换
    ...
    for i = 1 , cbCardCount do
        print("扑克：",i,cbCardData[i],cbCardCount) 
...
function GameLogic:AnalyseCard(cbCardIndex,WeaveItem,cbWeaveCount)
    ...
    assert((cbCardCount >=2 and cbCardCount <= 17) and (cbCardCount - 2)%3 == 0)
    if cbCardCount < 2 or cbCardCount > 17 or  (cbCardCount - 2)%3 ~= 0 then
        return false
    end
