...
function TableLayer:doAction(action,pBuffer)
    ...
    elseif action == NetMsgId.REC_SUB_S_BETTING then
        ...
        if GameCommon:getRoleChairID() ~= GameCommon.wBankerUser then
            ...
            local tableBettingType = {5,10,15,20,30,40,50}
            if GameCommon.gameConfig.bBettingType == 4 then
                tableBettingType = {10,20,30,40,60,80,100}
            end
            -- if wChairID == GameCommon:getRoleChairID() then
            --     if GameCommon.gameConfig.bBettingType == 1 then
            --         table.insert(tableBettingType,#tableBettingType+1)
            --     end
            ...
            for key, var in pairs(tableBettingType) do
                local img = string.format("puke/addscore/addscore_%d.png",var)
                -- if GameCommon.gameConfig.bBettingType == 1 and var == 5 then
...
function TableLayer:showHandCard(wChairID,effectsType,cbValueType)
    ...
    for i = 1, 3 do
        ...
        if effectsType == 1 then
            local pt = cc.p(0 + beganX + (i-1)*stepX, 0)
...
function TableLayer:isLightCardType()
    if StaticData.Hide[CHANNEL_ID].btn15 == 0 or GameCommon.gameConfig.bCuopai ~= 1  then
        return false
    end
