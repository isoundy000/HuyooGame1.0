...
local SanTableLayer = require("game.puke.SanTableLayer")
...
function GameLayer:startGame(...)
    ...
    local csb = cc.CSLoader:createNode("GameLayerSan.csb")
    self:addChild(csb)
    self.root = csb:getChildByName("Panel_root")
    self.csb = csb     
    GameCommon:init()
    GameCommon.MAX_COUNT = 15
    self.tableLayer = SanTableLayer:create(self.root)
        ...
        elseif subCmdID == NetMsgId.REC_SUB_S_GAME_END then
            for i = 0 , GameCommon.gameConfig.bPlayerCount-1 do
                if GameCommon.player[i] ~= nil then
                    ...
                    self.tableLayer:doAction(NetMsgId.REC_SUB_S_GAME_END,{wChairID = i, lGameScore = pBuffer.lGameScore[i],fWriteScoreArr =pBuffer.fWriteScoreArr[i]})
