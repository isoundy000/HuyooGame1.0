...
function GameEndLayer:onCreate(pBuffer) 
    ...
    uiButton_look:setPressedActionEnabled(true)
    local function onEventContinueLook(sender,event)
        if event == ccui.TouchEventType.ended then
            Common:palyButton()
    ...
    uiButton_look:addTouchEventListener(onEventContinueLook)    
...
function GameEndLayer:showMingTang(uiListView_mingTang,wChiHuKind)
    ...
    self.CHD_GANG_SHANG_PAO =         0x0001                              --杠上炮
    self.CHD_QIANG_GANG   =           0x0002                              --抢杠
    self.CHD_GANG_KAI     =           0x0004                              --杠开
    self.CHD_SHI_SAN_LAN  =           0x0008                              --十三烂
    self.CHD_QI_SHI_SAN_LAN =         0x0010                              --七星十三烂
    self.CHD_DAN_DIAO      =          0x0020                              --单吊
    self.CHD_MEN_QING     =           0x0040                              --门清
    self.CHD_DA_QI_DUI   =            0x0080                              --大七对
    self.CHD_XIAO_QI_DUI  =           0x0100                              --小七对
    self.CHD_QING_YI_SE =             0x0200                              --清一色
    self.CHD_ZI_YI_SE  =              0x0400                              --字一色
    self.CHD_TIAN_HU  =               0x0800                              --天胡
    self.CHD_DI_HU    =               0x1000                              --地胡 
    self.CHD_HAO_QI_XIAO_DUI   =      0x2000                              --豪华七小对
    self.CHD_CHAO_HAO_QI_XIAO_DUI =   0x4000                              --超豪华七小对
    
    
    --杠上炮
    if Bit:_and(wChiHuKind,self.CHD_GANG_SHANG_PAO) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_gangshangpao.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --抢杠
    if Bit:_and(wChiHuKind,self.CHD_QIANG_GANG) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qiangganghu.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --杠开
    if Bit:_and(wChiHuKind,self.CHD_GANG_KAI) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_gangshangkaihua.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --十三烂
    if Bit:_and(wChiHuKind,self.CHD_SHI_SAN_LAN) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_shisanlan.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end    
    --七星十三烂
    if Bit:_and(wChiHuKind,self.CHD_QI_SHI_SAN_LAN) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qixingshisanlan.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --单钓
    if Bit:_and(wChiHuKind,self.CHD_DAN_DIAO) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_dandiao.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --门清
    if Bit:_and(wChiHuKind,self.CHD_MEN_QING) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_menqing.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --大七对
    if Bit:_and(wChiHuKind,self.CHD_DA_QI_DUI) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_daqidui.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end   
    --小七对
    if Bit:_and(wChiHuKind,self.CHD_XIAO_QI_DUI) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qixiaodui.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end    
    --清一色
    if Bit:_and(wChiHuKind,self.CHD_QING_YI_SE) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qingyise.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --字一色
    if Bit:_and(wChiHuKind,self.CHD_ZI_YI_SE) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_ziyise.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --天胡
    if Bit:_and(wChiHuKind,self.CHD_TIAN_HU) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_tianhu.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --地胡
    if Bit:_and(wChiHuKind,self.CHD_DI_HU) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_dihu.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end    
    --豪华七小对
    if Bit:_and(wChiHuKind,self.CHD_HAO_QI_XIAO_DUI) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_haohuaqixiaodui.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end    
    --超豪华七小对
    if Bit:_and(wChiHuKind,self.CHD_CHAO_HAO_QI_XIAO_DUI) ~= 0 then
