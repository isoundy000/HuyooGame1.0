...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_MAIFEN then           --买飘数据接受
            _tagMsg.pBuffer.UserMaiFenAction = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.UserMaiFenAction[i]= luaFunc:readRecvBool()      --是否已做处理
                print("买飘数据1：",i,_tagMsg.pBuffer.UserMaiFenAction[i])
            end 
            _tagMsg.pBuffer.UserMaiFenType = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.UserMaiFenType[i]= luaFunc:readRecvByte()      --买分类型
                print("买飘数据2：",i,_tagMsg.pBuffer.UserMaiFenType[i])
            end 
            _tagMsg.pBuffer.UserMaiFenCount = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.UserMaiFenCount[i]= luaFunc:readRecvByte()      --买分数据
                print("买飘数据3：",i,_tagMsg.pBuffer.UserMaiFenCount[i])
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            ...
            _tagMsg.pBuffer.wChiFengJianCard = {}                       --风箭吃牌操作结果
            for i = 1 , 3 do
                _tagMsg.pBuffer.wChiFengJianCard[i] = luaFunc:readRecvByte()
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            for i = 1 , 4 do
                ...
                for j = 1 , 4 do
                    ...
                    _tagMsg.pBuffer.WeaveItemArray[i][j].wChiFengJianCard = {}                  --吃风箭数据
                    for n = 1 , 3 do
                        _tagMsg.pBuffer.WeaveItemArray[i][j].wChiFengJianCard[n] = luaFunc:readRecvByte() 
                    end 
                end
            end
        ...
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
            ...
            for i = 1,4 do
                ...
                for j = 1 , 4 do
                    ...
                    _tagMsg.pBuffer.WeaveItemArray[i][j].cbPublicCard = luaFunc:readRecvByte()  --公开标志
                    _tagMsg.pBuffer.WeaveItemArray[i][j].wProvideUser = luaFunc:readRecvWORD()  --供应用户
                    print("组合数据：",i,j, _tagMsg.pBuffer.WeaveItemArray[i][j].cbWeaveKind ,_tagMsg.pBuffer.WeaveItemArray[i][j].cbCenterCard, _tagMsg.pBuffer.WeaveItemArray[i][j].cbPublicCard ) 
                    _tagMsg.pBuffer.WeaveItemArray[i][j].wChiFengJianCard = {}                  --吃风箭数据
                    for n = 1 , 3 do
                        _tagMsg.pBuffer.WeaveItemArray[i][j].wChiFengJianCard[n] = luaFunc:readRecvByte() 
                    end 
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START_MAJIANG then
            ...
            if GameCommon.gameConfig.bPlayerCount == 2 then
                self.tableLayer:updateLeftCardCount(136-36-13*GameCommon.gameConfig.bPlayerCount-1)
            else
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_MAIFEN then             --接受买飘数据（宜春麻将） 
                    
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                GameCommon.player[i-1].UserMaiFenAction = pBuffer.UserMaiFenAction[i]
                GameCommon.player[i-1].UserMaiFenType = pBuffer.UserMaiFenType[i]
                GameCommon.player[i-1].UserMaiFenCount = pBuffer.UserMaiFenCount[i]
            end    
            self:updatePlayerPiaoFen(0)    
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))           
        elseif subCmdID == NetMsgId.SUB_S_OUT_CARD_NOTIFY_MAJIANG then             --用户提示出牌
            ...
            uiPanel_end:stopAllActions()
                ...
                cc.CallFunc:create(function(sender,event)          
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.majiang.65.GameEndLayer"):create(pBuffer))  
                    end            
                end),
...
--                            GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            print("麻将列表：",1,pBuffer.cbCardData[1]) 
            if pBuffer.cbCardData[1] ~= 0 then
                self:updatePlayerPiaoFen(1)                 
            else
                self:updatePlayerPiaoFen(0) 
            end 
...
function GameLayer:updatePlayerlfatigue()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.tableConfig.lFatigueValue~= nil then
            local fatigue =GameCommon.tableConfig.lFatigueValue[i]
            if fatigue ~= 0 then 
...
function GameLayer:updatePlayerReady()
    ...
    if num == GameCommon.gameConfig.bPlayerCount then 
        self:updatePlayerPiaoFen(0)  
    end 
end

function GameLayer:updatePlayerPiaoFen(data)       
    if GameCommon.gameConfig == nil or GameCommon.gameConfig.bMaiPiaoCount == 0 then
        return
    end   
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil  then
            ...
            if GameCommon.player[wChairID].UserMaiFenAction == false or data == 1 then
                if GameCommon.player[wChairID].UserMaiFenCount == 0 then
                    uiImage_piao:loadTexture("game/mj_piaofen_1.png")
                elseif GameCommon.player[wChairID].UserMaiFenCount == 1 then
                    uiImage_piao:loadTexture("game/pukenew_score_1.png")
                elseif GameCommon.player[wChairID].UserMaiFenCount == 2 then
                    uiImage_piao:loadTexture("game/pukenew_score_2.png")
                elseif GameCommon.player[wChairID].UserMaiFenCount == 3 then
                    uiImage_piao:loadTexture("game/pukenew_score_3.png")
                elseif GameCommon.player[wChairID].UserMaiFenCount == 4 then
                    uiImage_piao:loadTexture("game/pukenew_score_4.png")
                elseif GameCommon.player[wChairID].UserMaiFenCount == 5 then
                ...
                uiImage_piao:addChild(img,1000)
                img:setPosition(img:getParent():getContentSize().width/2,img:getParent():getContentSize().height/2+img:getContentSize().height)
                if GameCommon.player[wChairID].UserMaiFenType == 1 then
                    img:setVisible(true)                   
                else 
    ...
    local wChairID = GameCommon:getRoleChairID()
    if  GameCommon.player[wChairID].UserMaiFenAction == false  then
        uiPanel_maipiao:setVisible(false) 
    end
