...
function GameEndLayer:onCreate(pBuffer)   
    ...
    for i = 1,GameCommon.gameConfig.bPlayerCount do
        ...
        if  pBuffer.mPiaoCount[i] == 0 then
        else
            uiImage_piaoFen:loadTexture(string.format("game/pukenew_score_%s.png",pBuffer.mPiaoCount[i]))
        end

        --连庄胡
        if pBuffer.bGenZhuangFlag == true then
            local item = ccui.ImageView:create("majiang/table/mingtang_gengzhuang.png")
            uiListView_mingTang:pushBackCustomItem(item)
        end

        if pBuffer.wWinner[i] == true then 
            self:showWCWD(uiListView_mingTang, pBuffer.mUserWCWDAction)
        end 
...
function GameEndLayer:showMingTang(uiListView_mingTang,wChiHuKind)
    ...
    if Bit:_and(wChiHuKind,GameCommon.CHR_QING_YI_SE) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qingshuihu2.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end

    --清一色
    if Bit:_and(wChiHuKind,GameCommon.CHR_QUAN_QIU_REN) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qingyise2.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end

    --七小对
    if Bit:_and(wChiHuKind,GameCommon.CHK_QI_XIAO_DUI) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_qixiaodui2.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end

    --龙七小对
    if Bit:_and(wChiHuKind,GameCommon.CHK_QI_XIAO_DUI_HAO) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_haohuaqixiaodui2.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end

    --碰碰胡
    if Bit:_and(wChiHuKind,GameCommon.CHK_PENG_PENG) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_pengpenghu2.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end

end

function GameEndLayer:showWCWD(uiListView_mingTang,mUserWCWDAction)
    if mUserWCWDAction == nil or mUserWCWDAction == 0 then 
        return
    end 
    --王钓
    if Bit:_and(mUserWCWDAction,0x01) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_wd.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --王闯
    if Bit:_and(mUserWCWDAction,0x02) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_wc.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --王钓王
    if Bit:_and(mUserWCWDAction,0x04) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_wdw.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
    --王闯王
    if Bit:_and(mUserWCWDAction,0x08) ~= 0 then
        local item = ccui.ImageView:create("majiang/table/mingtang_wcw.png")
        uiListView_mingTang:pushBackCustomItem(item)
    end
end
