...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_SEND_PIAO_RESULTWD then      --接受买飘数据（王钓麻将）
            _tagMsg.pBuffer.mPiaoCount = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.mPiaoCount[i]= luaFunc:readRecvByte()      --是否已做处理
                print("买飘数据1：",i,_tagMsg.pBuffer.mPiaoCount[i])
            end             
            _tagMsg.pBuffer.mPiaoUser = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.mPiaoUser[i]= luaFunc:readRecvBool()      --是否已做处理    false 没操作  true  已操作不做操作
                print("买飘数据2：",i,_tagMsg.pBuffer.mPiaoUser[i])
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_WCWD  then              --通知客户端进行王闯王钓操作  
            _tagMsg.pBuffer.mWCWDUser = luaFunc:readRecvWORD() 						--王闯王钓用户
            _tagMsg.pBuffer.mWCWDOperate = luaFunc:readRecvWORD() 					--操作码
            _tagMsg.pBuffer.mCode = luaFunc:readRecvByte()
            	                        --是否已操作
        elseif subCmdID == NetMsgId.CMD_S_WCWDSendCard  then       --王闯王钓拿牌
            _tagMsg.pBuffer.cbCardData = luaFunc:readRecvByte()	                --麻将数据
            _tagMsg.pBuffer.GetCardUser = luaFunc:readRecvWORD()				--王闯王钓拿牌玩家
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            _tagMsg.pBuffer.mUserWCWDAction = luaFunc:readRecvWORD()   			--王钓、王闯、王钓王、王闯王
            _tagMsg.pBuffer.bGenZhuangFlag =  luaFunc:readRecvBool()    --跟庄胡   true 跟庄  false 不跟

            _tagMsg.pBuffer.mPiaoCount = {}--飘分
            for i = 1, 4 do
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
	        ...
	        _tagMsg.pBuffer.mWCWDOperate = luaFunc:readRecvWORD()					--操作码
            _tagMsg.pBuffer.mWCWDUser = luaFunc:readRecvWORD()						--王闯王钓用户
            _tagMsg.pBuffer.mUserWCWDActionEx = luaFunc:readRecvWORD()				--断线重连王闯王钓胡判断用户	
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START_MAJIANG then
            ...
            if GameCommon.gameConfig.bPlayerCount == 2 and GameCommon.gameConfig.bWuTong == 0 then
                ...
                self.tableLayer:updateLeftCardCount(108+4-36-13*GameCommon.gameConfig.bPlayerCount-1)
            else
                self.tableLayer:updateLeftCardCount(108+4-13*GameCommon.gameConfig.bPlayerCount-1)
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_WCWD  then              --通知客户端进行王闯王钓操作  
            self.tableLayer:doAction(NetMsgId.SUB_S_WCWD, pBuffer)
        elseif subCmdID == NetMsgId.CMD_S_WCWDSendCard  then       --王闯王钓拿牌
            self.tableLayer:doAction(NetMsgId.CMD_S_WCWDSendCard, pBuffer)
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            self.tableLayer:doAction(NetMsgId.SUB_S_GAME_END_MAJIANG,{wWinner = pBuffer.wWinner,wProvideUser = pBuffer.wProvideUser,mUserWCWDAction = pBuffer.mUserWCWDAction})
            ...
            cc.CallFunc:create(function(sender,event)          
                if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                    ...
                    uiPanel_end:addChild(require("game.majiang.67.GameEndLayer"):create(pBuffer))                         
                end 
            end),
...
--                            GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
            ...
            if pBuffer.cbActionMask ~= 0 then
                if pBuffer.wOutCardUser == GameCommon:getRoleChairID() or pBuffer.wOutCardUser == 65535 then
                    self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask,
                        cbActionCard = pBuffer.cbActionCard,bIsSelf = true,cbGangCard = pBuffer.cbGangCard, wLastOutCardUser = pBuffer.wLastOutCardUser,mUserWCWDActionEx =  pBuffer.mUserWCWDActionEx})
                else
                    self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask,
                        cbActionCard = pBuffer.cbActionCard,bIsSelf = false,cbGangCard = pBuffer.cbGangCard,wLastOutCardUser = pBuffer.wLastOutCardUser,mUserWCWDActionEx =  pBuffer.mUserWCWDActionEx})
                end
            end
        ...
        --    end
            ...
            if pBuffer.mWCWDOperate ~= 0 then
                local viewID = GameCommon:getViewIDByChairID(pBuffer.mWCWDUser)
                if viewID == 1 then 
...
function GameLayer:updatePlayerReady()
    ...
    if GameCommon.tableConfig.wCurrentNumber ~= nil and GameCommon.tableConfig.wCurrentNumber == 0 then
        --cc.UserDefault:getInstance():setIntegerForKey("UserDefault_DistanceAlarm",0)
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil  then
            ...
            if GameCommon.player[wChairID].mPiaoUser == true  then
                if GameCommon.player[wChairID].mPiaoCount == 0 then
                    uiImage_piao:loadTexture("common/hall_5.png")
                ...
                elseif GameCommon.player[wChairID].mPiaoCount == 10 then
                    uiImage_piao:loadTexture("game/pukenew_score_10.png")
                else
                    uiImage_piao:setVisible(false)
                end        
            end  
