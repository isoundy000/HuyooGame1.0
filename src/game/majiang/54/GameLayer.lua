...
local TableLayer = require("game.majiang.TableLayer1")
...
function GameLayer:startGame(...)
    ...
    GameCommon.mHunCard = nil
    
    local visibleSize = cc.Director:getInstance():getVisibleSize()
    local csb = cc.CSLoader:createNode("GameZMDMaJiang.csb")
    self:addChild(csb)
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_START_MAJIANG then
            ...
            _tagMsg.pBuffer.wBankerUser = luaFunc:readRecvWORD()        --庄家用户
            print("庄家用户：",_tagMsg.pBuffer.wBankerUser)
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_XIAPAO then
            _tagMsg.pBuffer.mXiaPaoCount = {}  
            for i = 1 , 4 do
                _tagMsg.pBuffer.mXiaPaoCount[i]= luaFunc:readRecvByte()
            end
            _tagMsg.pBuffer.mXiaPaoUser = {}     
            for i = 1 , 4 do              
                _tagMsg.pBuffer.mXiaPaoUser[i]= luaFunc:readRecvBool()
            end
        elseif subCmdID == NetMsgId.SUB_S_SEND_HUN_CARD then            --发送混牌
            _tagMsg.pBuffer.mHunCard= luaFunc:readRecvByte()
--            GameCommon.mHunCard =_tagMsg.pBuffer.mHunCard
--            return true
        elseif subCmdID == NetMsgId.SUB_S_SEND_BAOTING_CARD then            --发送报听操作
            _tagMsg.pBuffer.wBaoTingUser = luaFunc:readRecvWORD()       --报听用户
            _tagMsg.pBuffer.mBaoTingCard = {}                           --报听可删牌
            for i = 1 , 34 do  
                _tagMsg.pBuffer.mBaoTingCard[i]= luaFunc:readRecvByte()
            end 
            _tagMsg.pBuffer.bBaoTingUserTF = {}                         --用户是否报听   true 报听完成    false 需要报听
            for i = 1 , 4 do              
                _tagMsg.pBuffer.bBaoTingUserTF[i]= luaFunc:readRecvBool()
            end
            _tagMsg.pBuffer.bAction= luaFunc:readRecvBool()
            print("发送报听操作1：",_tagMsg.pBuffer.wBaoTingUser,_tagMsg.pBuffer.bBaoTingUserTF[_tagMsg.pBuffer.wBaoTingUser+1])
        elseif subCmdID == NetMsgId.SUB_S_SEND_BAOTING_ERROR then            --报听后出牌错误
            require("common.MsgBoxLayer"):create(0,nil,"不符合出牌规则")
            return true
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
            ...
            _tagMsg.pBuffer.mHunCard = luaFunc:readRecvByte()                   --混牌
            
            _tagMsg.pBuffer.wBaoTingUser = luaFunc:readRecvWORD()       --报听用户
            print("报听用户：",_tagMsg.pBuffer.wBaoTingUser)
            _tagMsg.pBuffer.mBaoTingCard = {}                           --报听可删牌
            for i = 1 , 34 do  
                _tagMsg.pBuffer.mBaoTingCard[i]= luaFunc:readRecvByte()
            end 
            ...
            for i = 1 , 4 do              
                _tagMsg.pBuffer.bBaoTingUserTF[i]= luaFunc:readRecvBool()  
                print("用户是否报听：",i,_tagMsg.pBuffer.bBaoTingUserTF[i])              
            end
            _tagMsg.pBuffer.bBaoTingAction = {}
            for i = 1 , 4 do              
                _tagMsg.pBuffer.bBaoTingAction[i]= luaFunc:readRecvBool()  
                print("用户报听：",i,_tagMsg.pBuffer.bBaoTingAction[i])              
            end
--            _tagMsg.pBuffer.bAction = luaFunc:readRecvBool()
--            print("用户报听：",_tagMsg.pBuffer.bAction)             
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START_MAJIANG then
            ...
            if GameCommon.gameConfig.bDFFlag == 1 then
                self.tableLayer:updateLeftCardCount(136-13*GameCommon.gameConfig.bPlayerCount-1)
            else
                self.tableLayer:updateLeftCardCount(120-13*GameCommon.gameConfig.bPlayerCount-1)
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_XIAPAO then             --用户下跑（飘分）                     
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                GameCommon.player[i-1].mPiaoCount = pBuffer.mXiaPaoCount[i]
                GameCommon.player[i-1].mPiaoUser = pBuffer.mXiaPaoUser[i]
            end    
            self:updatePlayerPiaoFen()    
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))  
        elseif subCmdID == NetMsgId.SUB_S_SEND_HUN_CARD then
            GameCommon.mHunCard = pBuffer.mHunCard            
            self:updateHunCard() 
            self:runAction(cc.Sequence:create(cc.DelayTime:create(1),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        elseif subCmdID == NetMsgId.SUB_S_SEND_BAOTING_CARD then            --发送报听操作         
            GameCommon.wBaoTingUser = pBuffer.wBaoTingUser                  --报听用户                         
            GameCommon.mBaoTingCard = pBuffer.mBaoTingCard                  --报听可删牌                    
            GameCommon.bBaoTingUserTF = pBuffer.bBaoTingUserTF              --用户是否报听   true 报听完成    false 需要报听
            GameCommon.bAction = pBuffer.bAction                            --用户报听   true 用户报听按钮显示    false 不显示
            print("发送报听操作2：",GameCommon.wBaoTingUser)
            self:updateBaoTing()  
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        elseif subCmdID == NetMsgId.SUB_S_OUT_CARD_NOTIFY_MAJIANG then             --用户提示出牌
        ...
        elseif subCmdID == NetMsgId.SUB_S_OUT_CARD_RESULT then             --用户出牌
            ...
            if GameCommon.bAction == true and GameCommon.wBaoTingUser ~= 65535 then                          
				...
				self:updateBaoTing(false)
			elseif GameCommon.bAction == false and GameCommon.wBaoTingUser ~= 65535 then  
                GameCommon.wBaoTingUser = 65535
                self:updateBaoTing(false)
            end
            self:updatehandplate()
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG then              --操作提示
            GameCommon.cbActionMask = pBuffer.cbActionMask       --动作掩码
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束          
            ...
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                if GameCommon:getViewIDByChairID(i-1) == 1 then
                else
                    self.tableLayer:setHandCard(i-1,GameCommon.player[i-1].cbCardCount, pBuffer.cbCardData[i])
            ...
            uiPanel_end:runAction(cc.Sequence:create(
                ...
                cc.CallFunc:create(function(sender,event) 
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.majiang.54.GameEndLayer"):create(pBuffer))
                    end            
                end),
...
--                            GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            GameCommon.mBaoTingCard = pBuffer.mBaoTingCard                  --报听可删牌                    
            GameCommon.bBaoTingUserTF = pBuffer.bBaoTingUserTF              --用户是否报听   true 报听完成    false 需要报听   
            if GameCommon.wBaoTingUser == GameCommon:getRoleChairID()  then          
                GameCommon.bAction = pBuffer.bBaoTingAction[GameCommon.wBaoTingUser+1]
            else
                GameCommon.bAction = false
            end 
            print("发送报听操作2：",GameCommon.wBaoTingUser)                        
            ...
            if pBuffer.cbActionMask ~= 0 then
                ...
                if pBuffer.wOutCardUser == GameCommon:getRoleChairID() or pBuffer.wOutCardUser == 65535 then
                    self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask, 
                        cbActionCard = pBuffer.cbActionCard,bIsSelf = true,cbGangCard = pBuffer.cbGangCard})
                else
                    self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask, 
...
function GameLayer:updatePlayerPiaoFen()    
    if GameCommon.gameConfig == nil or GameCommon.gameConfig.bDXPFlag == 0 then
        return
    end   
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil  then
            ...
            if GameCommon.player[wChairID].mPiaoUser == true then
                if GameCommon.player[wChairID].mPiaoCount == 0 then
                    uiImage_piao:loadTexture("game/game_budai.png")
                ...
                elseif GameCommon.player[wChairID].mPiaoCount == 4 then
                    uiImage_piao:loadTexture("game/pukenew_score_4.png")
                elseif GameCommon.player[wChairID].mPiaoCount == 5 then
                    uiImage_piao:loadTexture("game/pukenew_score_5.png")
                else
    ...
    if  GameCommon.player[wChairID].mPiaoUser == false and GameCommon.gameConfig.bDXPFlag == 1 then
        uiPanel_piaoFen:setVisible(true)
    else
        uiPanel_piaoFen:setVisible(false)        
    end
    print("飘分判断",GameCommon.player[wChairID].mPiaoUser,wChairID,GameCommon.player[wChairID].mPiaoCount,GameCommon.gameConfig.bDXPFlag)

end

function GameLayer:updateHunCard()
    if GameCommon.gameConfig == nil or GameCommon.gameConfig.bDHPlayFlag == 0 then 
        return
    end     
    self.tableLayer:setHandCard(GameCommon:getRoleChairID(),GameCommon.player[GameCommon:getRoleChairID()].cbCardCount, GameCommon.player[GameCommon:getRoleChairID()].cbCardData)

    local uiPanel_HunCard = ccui.Helper:seekWidgetByName(self.root,"Panel_HunCard")
    uiPanel_HunCard:setVisible(true)
    local uiImage_HunCard = ccui.Helper:seekWidgetByName(self.root,"Image_HunCard")  
    local cbValue = Bit:_and(GameCommon.mHunCard,0x0F)
    local cbColor = Bit:_rshift(Bit:_and(GameCommon.mHunCard,0xF0),4)
    local cardIndex = cc.UserDefault:getInstance():getIntegerForKey(Default.UserDefault_MaJiangCard,0)
    if cardIndex ~= 0 then
        uiImage_HunCard:loadTexture(string.format("majiang/card1/card1/hand_card%d%d.png",cbColor,cbValue))
    else
        uiImage_HunCard:loadTexture(string.format("majiang/card1/card0/hand_card%d%d.png",cbColor,cbValue))
    end
    self.tableLayer:showHandCard(GameCommon:getRoleChairID(),0)
end 


function GameLayer:updateBaoTing(data)   --报听判断
    if GameCommon.gameConfig == nil or GameCommon.gameConfig.bBTHu == 0   then
        return
    end    
    ...
    if GameCommon.wBaoTingUser == GameCommon:getRoleChairID()  then 
        if  GameCommon.bBaoTingUserTF[GameCommon.wBaoTingUser+1] == true or GameCommon.bAction == false then         
            uiPanel_siShou:setVisible(false)
        else  
            uiPanel_siShou:setVisible(true)
        end 
    end 
    print("发送报听操作3：",GameCommon.wBaoTingUser,GameCommon:getRoleChairID(),GameCommon.bBaoTingUserTF[GameCommon.wBaoTingUser+1],GameCommon.bAction) 
    if GameCommon.wBaoTingUser ~= 65535 and  GameCommon.bBaoTingUserTF[GameCommon.wBaoTingUser+1] == true and  GameCommon.bAction == false then 
       print("播放声音：",GameCommon.wBaoTingUser) 
       require("common.Common"):playEffect("common/huangzhuang.mp3")   
    end 
    local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,"Panel_player")
    for i = 1 , GameCommon.gameConfig.bPlayerCount do    
        local viewID = GameCommon:getViewIDByChairID(i-1)
        print("发送报听操作4：",i,GameCommon.bBaoTingUserTF[viewID],viewID)
        if GameCommon.bBaoTingUserTF ~= nil and GameCommon.bBaoTingUserTF[viewID] ~= nil  then           
            local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
            local uiImage_piao = ccui.Helper:seekWidgetByName(uiPanel_player,"Image_piao")
            if GameCommon.bBaoTingUserTF[i] == true then  
                uiImage_piao:setVisible(true)       
                uiImage_piao:loadTexture("game/game_baotingtishi.png")                
            end  
        end
    end
