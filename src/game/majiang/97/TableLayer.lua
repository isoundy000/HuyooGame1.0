...
local GameOpration = require("game.majiang.97.GameOpration")
local GameGangOpration = require("game.majiang.97.GameGangOpration")
local GameSpecial = require("game.majiang.97.GameSpecial")
...
function TableLayer:showMoveCard(data)  --- 手牌移动
    for i =1 , GameCommon.gameConfig.bPlayerCount do
        if GameCommon.gameConfig.bPlayerCount == 2 and  i == 2  then 
            i = 3 
        end  
        ...
        if uiPanel_discardCard then
            ...
            for i = 1 ,#nodeArr do 
                nodeArr[i]:setColor(cc.c3b(255,255,255))
                if data ~= nil and data == nodeArr[i].data then 
                    nodeArr[i]:setColor(cc.c3b(170,170,170))
                end 
            end 
        end

        local uiPanel_discardlaiCard = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_discardlaiCard%d",i))
        if uiPanel_discardlaiCard then
            local nodeArr = uiPanel_discardlaiCard:getChildren()
            for i = 1 ,#nodeArr do 
                nodeArr[i]:setColor(cc.c3b(255,255,255))
                if data ~= nil and data == nodeArr[i].data then 
                    nodeArr[i]:setColor(cc.c3b(170,170,170))
        ...
        if uiPanel_weaveItemArray then
            local nodeArr = uiPanel_weaveItemArray:getChildren()
            for i = 1 ,#nodeArr do 
                local childLen = nodeArr[i]
                local aa = childLen:getChildren()
                for j = 1 , #aa do 
                    aa[j]:setColor(cc.c3b(255,255,255))
                    if data ~= nil and data == aa[j].data then 
                        aa[j]:setColor(cc.c3b(170,170,170))
                    end   
                end 
            end  
        end 
...
--            end)))  
    ...
    elseif action == NetMsgId.SUB_S_SEND_CARD_MAJIANG then
        ...
        print('--------->>>>>>>>添加手牌',wChairID,pBuffer.cbCardData)
        ...
        if pBuffer.wProvideUser >= GameCommon.gameConfig.bPlayerCount then
            ...
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                if pBuffer.wWinner[i] == true then
                    if pBuffer.mUserWCWDAction ~= nil and pBuffer.mUserWCWDAction ~= 0 then 
                        --王钓
                        if Bit:_and(pBuffer.mUserWCWDAction,0x01) ~= 0 then
                            GameCommon:playAnimation(self.root, "王钓",wChairID)
                        end
                        --王闯
                        if Bit:_and(pBuffer.mUserWCWDAction,0x02) ~= 0 then
                            GameCommon:playAnimation(self.root, "王闯",wChairID)
                        end
                        --王钓王
                        if Bit:_and(pBuffer.mUserWCWDAction,0x04) ~= 0 then
                            GameCommon:playAnimation(self.root, "王钓王",wChairID)
                        end
                        --王闯王
                        if Bit:_and(pBuffer.mUserWCWDAction,0x08) ~= 0 then
                            GameCommon:playAnimation(self.root, "王闯王",wChairID)
                        end
                        -- if pBuffer.mUserWCWDAction >= 0x02 then 
                        --     GameCommon:playAnimation(self.root, "王闯",wChairID)
                        -- else
                        --     GameCommon:playAnimation(self.root, "王钓",wChairID)
                        -- end 
            ...
            for k, v in pairs(cbCardList) do
                ...
                if card ~= nil then 
                    ...
                    if TS_gang == true then                  
                        ...
                        -- self.sCircle = cc.Sprite:create("newcommon/chao_left.png")
                        -- self.sCircle:setAnchorPoint(cc.p(0.5,0.5))
                        node:setColor(cc.c3b(0,0,0))
                        card:addChild(node)
                        node:setPosition(72,82)
                        node:start(0)
                    end 
                end 
            end
...
function TableLayer:addDiscardCard(wChairID, cbDiscardCard)
    ...
    local node = nil
    local pos = nil
    if cbDiscardCard == GameCommon.mLaiZiCard then   -- 癞子与其他弃牌添加不同位置
        table.insert(GameCommon.player[wChairID].cbDiscardlaiCard,#GameCommon.player[wChairID].cbDiscardlaiCard+1,cbDiscardCard) 
        node = self:setDiscardlaiCard(wChairID, GameCommon.player[wChairID].cbDiscardlaiCard)
        ...
        node = self:setDiscardCard(wChairID, GameCommon.player[wChairID].cbDiscardCount, GameCommon.player[wChairID].cbDiscardCard)
        pos = cc.p(node:getPosition())
    end 
    if node~= nil  then     
        node:stopAllActions()
...
function TableLayer:setRBDiscardCard(wChairID, cbDiscardCount, bDiscardCard)  --97游戏特列  弃牌重新分类
    GameCommon.player[wChairID].cbDiscardlaiCard = {}
    GameCommon.player[wChairID].cbDiscardCard = {}
    GameCommon.player[wChairID].cbDiscardCount = 0
    for i = 1, cbDiscardCount do
        if bDiscardCard[i] == GameCommon.mLaiZiCard then
            table.insert(GameCommon.player[wChairID].cbDiscardlaiCard,1,bDiscardCard[i]) 
        else
            GameCommon.player[wChairID].cbDiscardCount = GameCommon.player[wChairID].cbDiscardCount + 1 
            GameCommon.player[wChairID].cbDiscardCard[GameCommon.player[wChairID].cbDiscardCount] = bDiscardCard[i]
        end 
    end
    self:setDiscardCard(wChairID, GameCommon.player[wChairID].cbDiscardCount, GameCommon.player[wChairID].cbDiscardCard)
    self:setDiscardlaiCard(wChairID, GameCommon.player[wChairID].cbDiscardlaiCard)
end 
...
--添加癞子弃牌
function TableLayer:setDiscardlaiCard(wChairID, cbDiscardlaiCard)

    GameCommon.player[wChairID].cbDiscardlaiCard = cbDiscardlaiCard
    
    local viewID = GameCommon:getViewIDByChairID(wChairID)
    local uiPanel_discardlaiCard = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_discardlaiCard%d",viewID))
    uiPanel_discardlaiCard:removeAllChildren()
    local anchorPoint = uiPanel_discardlaiCard:getAnchorPoint()
    local size = uiPanel_discardlaiCard:getContentSize()
    local cbDiscardlaiCard = GameCommon.player[wChairID].cbDiscardlaiCard
    local maxRow = 4
    local lastNode = nil
    ...
    if viewID == 1 then
        ...
        local stepX = cardWidth
        local stepY = cardHeight-12
        for i = 1, #cbDiscardlaiCard do
            local row = math.floor((i-1)/maxRow)
            local line = (i-1)%maxRow
            local card = GameCommon:getDiscardCardAndWeaveItemArray(cbDiscardlaiCard[i],viewID)
            card.data = cbDiscardlaiCard[i]
            lastNode = card
            uiPanel_discardlaiCard:addChild(card)
            card:setScale(cardScale)
            card:setPosition(beganX + stepX*line ,beganY + stepY*row)
            card:setLocalZOrder(i)               
            if GameCommon.mLaiZiCard ~= 0 and GameCommon.mLaiZiCard == cbDiscardlaiCard[i] then
                -- local uiText_OfflineTime = ccui.Text:create("0","fonts/DFYuanW7-GB2312.ttf","16")
                -- uiText_OfflineTime:setName('Text_OfflineTime')
                -- uiText_OfflineTime:setTextColor(cc.c3b(251,201,99)) 
                -- uiText_OfflineTime:enableOutline(cc.c4b(101, 40, 14), 2)
...
function TableLayer:setHandCard(wChairID,cbCardCount,cbCardData)
    ...
    elseif GameCommon.tableConfig.wKindID == 97 then 

        local cbCardData_Tem = {}
        local mChaoTianCard = 0
        for i = 1, cbCardCount do
            if  GameCommon.mChaoTianCard~= 0 and cbCardData[i] == GameCommon.mChaoTianCard  then
                table.insert(cbCardData_Tem,1,cbCardData[i])
                mChaoTianCard = mChaoTianCard+1
            elseif GameCommon.mLaiZiCard~= 0 and cbCardData[i] == GameCommon.mLaiZiCard then
                    table.insert(cbCardData_Tem,mChaoTianCard+1,cbCardData[i])
            else
...
function TableLayer:addOneHandCard(wChairID, cbCard, pos)
    print("+++++++++获取亡牌位置-----",GameCommon.mChaoTianCard,GameCommon.mLaiZiCard,cbCard)
    local viewID = GameCommon:getViewIDByChairID(wChairID)
    local uiPanel_handCard = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_handCard%d",viewID))
    --插入手牌中
    ...
    elseif GameCommon.tableConfig.wKindID == 97 and (GameCommon.mChaoTianCard~= 0 and GameCommon.mLaiZiCard~= 0 and (cbCard == GameCommon.mChaoTianCard or cbCard == GameCommon.mLaiZiCard)) then
            local nnnn = 1
            if GameCommon.mChaoTianCard ~= 0 and  GameCommon.mChaoTianCard== cbCard then 
                --王牌麻将，王牌放左边
                table.insert(GameCommon.player[wChairID].cbCardData,1,cbCard)
            elseif GameCommon.mLaiZiCard ~= 0 and  GameCommon.mLaiZiCard== cbCard then 
                local mChaoTianCard = 0
                for i =1 ,3 do
                    if GameCommon.player[wChairID].cbCardData[i] == GameCommon.mChaoTianCard then 
                        mChaoTianCard = mChaoTianCard + 1
                    end 
                end 
                --王牌麻将，王牌放左边
                table.insert(GameCommon.player[wChairID].cbCardData,mChaoTianCard+1,cbCard)
            end
    else
        local isInsert = false
        if GameCommon.tableConfig.wKindID ~= 97 then
            for i = 1, GameCommon.player[wChairID].cbCardCount do
                if GameCommon.player[wChairID].cbCardData[i] and cbCard < GameCommon.player[wChairID].cbCardData[i] and ((GameCommon.tableConfig.wKindID ~= 45 and GameCommon.tableConfig.wKindID ~= 68 and GameCommon.tableConfig.wKindID ~= 63 and   GameCommon.tableConfig.wKindID ~= 67 ) or GameCommon.player[wChairID].cbCardData[i] ~= 0x31) then
            ...
            if GameCommon.mLaiZiCard ~= 0 and  GameCommon.mChaoTianCard ~= cbCard and GameCommon.mLaiZiCard ~= cbCard then 
                for i = 1, GameCommon.player[wChairID].cbCardCount do
                ...
                for i = 1, GameCommon.player[wChairID].cbCardCount do
                    if GameCommon.player[wChairID].cbCardData[i] and cbCard < GameCommon.player[wChairID].cbCardData[i] and GameCommon.player[wChairID].cbCardData[i] ~= GameCommon.mChaoTianCard  and GameCommon.player[wChairID].cbCardData[i] ~= GameCommon.mLaiZiCard then
                        table.insert(GameCommon.player[wChairID].cbCardData,i,cbCard)
                        isInsert = true
                        break
                    end
    ...
    elseif GameCommon.tableConfig.wKindID == 67 and cbCard == 0x31 then
        ...
        if GameCommon.tableConfig.wKindID ~= 97 then 
            ...
            local isInsert = false
            if GameCommon.mChaoTianCard == 0 and GameCommon.mLaiZiCard == 0 then 
                for key, var in pairs(GameCommon.player[wChairID].cardNode) do
                ...
                for key, var in pairs(GameCommon.player[wChairID].cardNode) do
                    if cbCard < var.data  and var.data ~= GameCommon.mChaoTianCard and var.data ~= GameCommon.mLaiZiCard  then
                        table.insert(GameCommon.player[wChairID].cardNode,key,data) 
                        isInsert = true
                        break
                    elseif GameCommon.mChaoTianCard ~= 0 and  GameCommon.mChaoTianCard== cbCard then 
                        --王牌麻将，王牌放左边
                        table.insert(GameCommon.player[wChairID].cardNode,1,data)
                        isInsert = true                      
                        break
                    elseif GameCommon.mLaiZiCard ~= 0 and  GameCommon.mLaiZiCard== cbCard then 
                        ...
                        --王牌麻将，王牌放左边
                        table.insert(GameCommon.player[wChairID].cardNode,mChaoTianCard+1,data)
                        isInsert = true                        
                        break
                    end
                end 
...
function TableLayer:showTianCard()
    ...
    if GameCommon.mLaiZiCard == nil or GameCommon.mLaiZiCard == 0  then 
        return
    end
    local card = GameCommon:GetCardHand(GameCommon.mLaiZiCard,0)
    uiPanel_tiancard:addChild(card)
    card:setPosition(80,50) 
    card:setScale(0.5)
    ...
    if viewID == 1 then
        ...
        --    local liang = ccui.ImageView:create("game/game_baotingkuang.png")
            ...
            if GameCommon.mBaoTingCard ~= nil and GameCommon.mBaoTingCard[1] ~= 0 then              
                for j = 1 , 14 do
                    local IsOpenTin = cc.UserDefault:getInstance():getBoolForKey('CDisOpenTin', true)
                    if GameCommon.mBaoTingCard[j] == GameCommon.player[wChairID].cardNode[i].data and IsOpenTin == true then                     
...
function TableLayer:initUI()
    ...
    local uiButton_piaofen5 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen5")        --飘一分 
    local uiButton_piaofen10 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen10")        --飘两分
    local uiButton_piaofen0 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen0")        --不飘
    Common:addTouchEventListener(uiButton_piaofen5,function() 
        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_PiaoFen,"b",5)
    end)

    Common:addTouchEventListener(uiButton_piaofen10,function() 
        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_PiaoFen,"b",10)
    end)

    Common:addTouchEventListener(uiButton_piaofen0,function() 
        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_PiaoFen,"b",0)
    end)
...
function TableLayer:updateGameState(state)
    ...
    elseif state == GameCommon.GameState_Start then
        ...
        local uiPanel_ready = ccui.Helper:seekWidgetByName(self.root,"Panel_ready")
        uiPanel_ready:setVisible(false)
        -- if GameCommon.tableConfig.wKindID == 67 or GameCommon.tableConfig.wKindID == 63 or GameCommon.tableConfig.wKindID == 45 or GameCommon.tableConfig.wKindID == 46 or GameCommon.tableConfig.wKindID == 50 and  StaticData.Hide[CHANNEL_ID].btn16 == 1  then 
        --     local uiButton_chakan = ccui.Helper:seekWidgetByName(self.root,"Button_chakan")
