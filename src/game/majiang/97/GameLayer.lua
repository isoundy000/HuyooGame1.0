...
local TableLayer = require("game.majiang.97.TableLayer")
...
function GameLayer:startGame(...)
    ...
    GameCommon.mChaoTianCard = 0   --朝天牌
    GameCommon.mLaiZiCard = 0      --癞子牌
    GameCommon.mSLYXScore = {}     --四癞喜分

    local visibleSize = cc.Director:getInstance():getVisibleSize()
    local csb = cc.CSLoader:createNode("GameLayerMaJiang97.csb")
    self:addChild(csb)
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            _tagMsg.pBuffer.mChengNum = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.mChengNum[i] = luaFunc:readRecvByte()--撑的次数  
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_CHAO_TIAN then  --起手发送朝天牌和癞子给客户端
            _tagMsg.pBuffer.mChaoTianCard = luaFunc:readRecvByte()  
            _tagMsg.pBuffer.mLaiZiCard = luaFunc:readRecvByte()  
            print("癞子天牌:","_tagMsg.pBuffer.mLaiZiCard",_tagMsg.pBuffer.mLaiZiCard,"_tagMsg.pBuffer.mChaoTianCard",_tagMsg.pBuffer.mChaoTianCard) 
        elseif subCmdID == NetMsgId.SUB_S_SI_LAI_YOU_XI then  --起手四癞有喜给客户端
            _tagMsg.pBuffer.mSLYXScore = {}
            for i = 1 , 4 do 
                _tagMsg.pBuffer.mSLYXScore[i] = luaFunc:readRecvLong()     --积分
            end 
        else
            --print("not found this subCmdID : %d",subCmdID)
            return false
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            cc.CallFunc:create(function(sender,event)          
                if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                    ...
                    uiPanel_end:addChild(require("game.majiang.97.GameEndLayer"):create(pBuffer))                         
                end 
            end),
...
--                            GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
        ...
        elseif subCmdID == NetMsgId.SUB_S_CHAO_TIAN then  --起手发送朝天牌和癞子给客户端
            ...
            self.tableLayer:showHandCard(GameCommon.meChairID,2)
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        elseif subCmdID == NetMsgId.SUB_S_SI_LAI_YOU_XI then  --起手四癞有喜给客户端
            GameCommon.mSLYXScore = {}
            for i = 1 , 4 do 
                GameCommon.mSLYXScore[i] = pBuffer.mSLYXScore[i]     --积分   四癞喜分
            end 
            self:updatePlayerInfo()
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_TIPS_MAJIANG then       --游戏胡牌提示
        ...
        if subCmdID == NetMsgId.SUB_GF_SCENE then
          ...
          --  self:updatePlayerPiaoFen()
            ...
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                self.tableLayer:setRBDiscardCard(i-1, pBuffer.cbDiscardCount[i], pBuffer.cbDiscardCard[i])
...
function GameLayer:updatePlayerInfo()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.mSLYXScore ~= nil and GameCommon.mSLYXScore[i] ~= nil then             
            if GameCommon.mSLYXScore[i]  ~= 0 then 
                ...
                uiPanel_player:addChild(uiText_OfflineTime,100)
                uiText_OfflineTime:setPosition(100,105)    
                if GameCommon.mSLYXScore[i] > 0 then             
                    uiText_OfflineTime:setString("四癞喜分+"..GameCommon.mSLYXScore[i])     
                else
                    uiText_OfflineTime:setString("四癞喜分"..GameCommon.mSLYXScore[i])        
                end 
                print("输出王牌显示++++++++++++",GameCommon.mSLYXScore[i])
            end 
        end 
    end
end
