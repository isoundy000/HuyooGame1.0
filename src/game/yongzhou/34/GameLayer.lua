...
local TableLayer = require("game.yongzhou.TableLayer")
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.RET_SC_GAME_CONFIG then
            ...
            if GameCommon.gameConfig ~= nil and GameCommon.gameConfig.bLaiZiCount >= 1 then
                GameCommon.weiCardType = 1
                GameCommon.tiCardType = 1
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            _tagMsg.pBuffer.cbCardCoutWW = luaFunc:readRecvByte()--王数量 
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY then
            ...
            _tagMsg.pBuffer.cbSubOperateCode = luaFunc:readRecvWORD()  --操作代码
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_OPERATE_RESULT then    
            local a = 1
         print("________________GGGGGGGGGGGGGGGGGGGG")       
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            _tagMsg.pBuffer.cbCardCoutWW = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.cbCardCoutWW[i] = luaFunc:readRecvByte()
            end
            ...
            _tagMsg.pBuffer.HuCardInfo.cbCurrentCard = luaFunc:readRecvByte()--当前摸牌
            _tagMsg.pBuffer.HuCardInfo.cbWWData = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.HuCardInfo.cbWWData[i] = luaFunc:readRecvByte()--当前摸牌
            end
            _tagMsg.pBuffer.HuCardInfo.cbCountWWInCardEye = luaFunc:readRecvByte()--当前摸牌
        ...
        elseif subCmdID == NetMsgId.SUB_S_WD then
            _tagMsg.pBuffer.wCurrentUser = luaFunc:readRecvWORD()     --当前用户
            _tagMsg.pBuffer.cbSubOperateCode = luaFunc:readRecvWORD()     --当前用户
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
            ...
            _tagMsg.pBuffer.bSubUserAction = luaFunc:readRecvWORD()                   --用户动作
            --弃牌
            ...
            for i = 1 , 4 do
                ...
                print("_tagMsg.pBuffer.bDiscardCardCount[%d] = ",i,_tagMsg.pBuffer.bDiscardCardCount[i])
            end 
            _tagMsg.pBuffer.cbCardCoutWW = luaFunc:readRecvByte()--王数量
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            GameCommon.player[wChairID].bUserCardCount = GameCommon.player[wChairID].bUserCardCount - pBuffer.cbCardCoutWW
            local cbCardIndex = GameLogic:SwitchToCardIndexs(pBuffer.cbCardData,GameCommon.player[wChairID].bUserCardCount)
            self.tableLayer:setHandCard(wChairID,GameCommon.player[wChairID].bUserCardCount, cbCardIndex, maxHanCardRow, pBuffer.cbCardCoutWW)
            ...
            if GameCommon.gameConfig.bDeathCard == 1 and GameCommon.gameConfig.bPlayerCount == 2  then
                self.tableLayer:updateLeftCardCount(60 - GameCommon.gameConfig.bPlayerCount*14 - 1 + GameCommon.gameConfig.bLaiZiCount, true)
            else
                self.tableLayer:updateLeftCardCount(80-GameCommon.gameConfig.bPlayerCount*14-1 + GameCommon.gameConfig.bLaiZiCount, true)
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_CLIENTERROR then
            ...
            self.tableLayer:setHandCard(wChairID, GameCommon.player[wChairID].bUserCardCount,pBuffer.cbCardIndex,10,pBuffer.cbCardCoutWW)
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_OPERATE_RESULT then 
            ...
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0.0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))        
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            GameCommon.uiPanel_showEndCard = ccui.Helper:seekWidgetByName(self.root,"Panel_showEndCard")
            GameCommon.uiPanel_showEndCard:setOpacity(0)
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.yongzhou.34.GameEndLayer"):create(pBuffer))
                    end
                    end),
...
--                                GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
                ...
                --黄庄
                local isWPei = false
                for key, var in pairs(pBuffer.cbCardCoutWW) do
                	if var > 0 then
                	   isWPei = true
                	end
                end
                if isWPei == true then
                    for i = 1 , GameCommon.gameConfig.bPlayerCount do
                        self.tableLayer:doAction(GameCommon.ACTION_WPei,{wAttachUser = i-1,wWcout = pBuffer.cbCardCoutWW[i],wMoney = pBuffer.lGameScore[i]})
                    end        
                end
                uiPanel_end:runAction(cc.Sequence:create(
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            self.tableLayer:setHandCard(wChairID,pBuffer.bUserCardCount[wChairID+1],pBuffer.cbCardIndex,7,pBuffer.cbCardCoutWW,true)
            self.tableLayer:showHandCard(wChairID,0)
            self:updateBankerUser()
            ...
            if pBuffer.wOutCardUser < GameCommon.gameConfig.bPlayerCount and pBuffer.cbOutCardData ~= 0 then
                if (pBuffer.bDispatch==true) then
                    if pBuffer.cbOutCardData == GameCommon.CardData_WW then
                        self.tableLayer:removeHandCard(pBuffer.wOutCardUser, pBuffer.cbOutCardData)
                        self.tableLayer:showHandCard(pBuffer.wOutCardUser,2)
                    end
...
function GameLayer:EVENT_TYPE_OPERATIONAL_OUT_CARD(event)
    ...
    if GameCommon.gameConfig.bLaiZiCount <= 0 then
        for i = 1, GameCommon.gameConfig.bPlayerCount do
            if i-1 ~= GameCommon:getRoleChairID() then
                for j = 1, GameCommon.player[i-1].bWeaveItemCount do
                    if GameCommon.player[i-1].WeaveItemArray[j].cbWeaveKind == GameCommon.ACK_WEI  and GameCommon.player[i-1].WeaveItemArray[j].cbCenterCard == cbCardData then
...
function GameLayer:updatePlayerInfo()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        --uiPanel_player:setVisible(true)     --永州牌桌特列
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil then
            ...
            if GameCommon.player[wChairID].cbOnline == 0x06 then
               ...
               uiImage_avatar:setColor(cc.c3b(140,140,140))
                -- if (CHANNEL_ID == 10 or CHANNEL_ID == 11) and GameCommon.gameConfig.bHostedTime ~= 0 then 
                ...
                --         uiText_OfflineTime:setPosition(30,40)   
                ...
                --     -- elseif GameCommon.player[wChairID].dwOfflineTime == 0 then 
