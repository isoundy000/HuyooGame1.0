...
function GameEndLayer:onCreate(pBuffer)    
    ...
    for key, var in pairs(GameCommon.player) do    
       ...
       -- local dwGold = Common:itemNumberToString(pBuffer.lGameScore[var.wChairID + 1])   
        ...
        --     local fatigue = pBuffer.lGameScore[var.wChairID + 1] * (GameCommon.tableConfig.wCellScore / GameCommon.tableConfig.wTableCellDenominator) 
        --     local uiText_fatigue = ccui.Helper:seekWidgetByName(root,"Text_fatigue")
        --     uiText_fatigue:setString(string.format(" 疲劳值:%0.2f",fatigue))
        -- end 
