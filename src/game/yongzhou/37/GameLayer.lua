...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
			...
			_tagMsg.pBuffer.cbCardCoutWW = luaFunc:readRecvByte()--王数量    Common:printLog(_tagMsg.pBuffer)
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            print("++++++111++++++++++",wChairID,GameCommon.player[wChairID].bUserCardCount)
            ...
            if GameCommon.gameConfig.bPlayerCount == 4 then
                self.tableLayer:updateLeftCardCount(80+GameCommon.gameConfig.bLaiZiCount-3*20-1, true)
            else
                self.tableLayer:updateLeftCardCount(80+GameCommon.gameConfig.bLaiZiCount-GameCommon.gameConfig.bPlayerCount*20-1, true)
            end
            self:updatePlayerlfatigue()
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.yongzhou.37.GameEndLayer"):create(pBuffer))
                    end
                    end),
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            if GameCommon.gameConfig.bPlayerCount == 4 and duijia == wChairID then
                self.tableLayer:setHandCard(GameCommon.wBankerUser,pBuffer.bUserCardCount[GameCommon.wBankerUser+1],pBuffer.cbCardIndex,10,pBuffer.cbCardCoutWW,true)   --用户   扑克数量  扑克数字   最大牌蹲列表  赖子数
                self.tableLayer:showHandCard(GameCommon.wBankerUser,0)
            else
                self.tableLayer:setHandCard(wChairID,pBuffer.bUserCardCount[wChairID+1],pBuffer.cbCardIndex,10,pBuffer.cbCardCoutWW,true)   --用户   扑克数量  扑克数字   最大牌蹲列表  赖子数
                self.tableLayer:showHandCard(wChairID,0)
            end
            self:updateBankerUser()
...
function GameLayer:updatePlayerInfo()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player == nil or GameCommon.player[wChairID] == nil then
            ...
            --     local fatigue =GameCommon.tableConfig.lFatigueValue[i]/ 100
            --     fatigue = Common:itemNumberToString(fatigue)
            --     uiText_fatigue:setString(tostring(fatigue))   
            -- end
...
function GameLayer:updatehandplate()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        print("+++++++++++++++++++",wChairID,GameCommon.player[wChairID].bUserCardCount)
