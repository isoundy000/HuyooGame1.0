...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            _tagMsg.pBuffer.wContinueWinCount = luaFunc:readRecvWORD()
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_TI_CARD then
            ...
            for i = 1 , 4 do
                _tagMsg.pBuffer.lUserScore[i] = luaFunc:readRecvLong()--游戏积分
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_WEI_CARD then
            ...
            _tagMsg.pBuffer.lUserActionCout = luaFunc:readRecvLong()
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_PENG_CARD then
            ...
            _tagMsg.pBuffer.lUserActionCout = luaFunc:readRecvLong()
            _tagMsg.pBuffer.m_bDispatch = luaFunc:readRecvBool()      --是否摸牌碰 
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY then
            ...
            _tagMsg.pBuffer.cbOperateCode = luaFunc:readRecvWORD()  --操作代码
             if (Bit:_and(_tagMsg.pBuffer.cbOperateCode,GameCommon.ACK_BIHU) ~= 0) then 
                print("必胡")
            elseif (Bit:_and(_tagMsg.pBuffer.cbOperateCode,GameCommon.ACK_CHIHU) ~= 0) then  
                print("胡")
             end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            _tagMsg.pBuffer.wContinueWinCount = luaFunc:readRecvWORD()--连庄数
            --成绩变量
            ...
            for j = 1 ,4 do
                _tagMsg.pBuffer.lUserScore[j] = luaFunc:readRecvLong()
            end
            _tagMsg.pBuffer.m_isAdd={}
            for j = 1 , GameCommon.gameConfig.bPlayerCount do
                _tagMsg.pBuffer.m_isAdd[j] = luaFunc:readRecvBool()
            end
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
            --游戏状态
            self.tableAddUserScore = {}
            for i = 1 ,4 do
                local ChairID = i - 1  
                self.tableAddUserScore[ChairID] = luaFunc:readRecvLong()
            end 
            _tagMsg.pBuffer.wContinueWinCount = luaFunc:readRecvWORD()
            GameCommon.wContinueWinCount = _tagMsg.pBuffer.wContinueWinCount
            --游戏信息
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            for i = 0, 3 do
                if GameCommon.player[i] ~= nil then
                    if i == pBuffer.wBankerUser then
                         GameCommon.player[i].bUserCardCount = 15
                    else
                        GameCommon.player[i].bUserCardCount = 14
                    end
                end
            end
            GameCommon.wContinueWinCount = _tagMsg.pBuffer.wContinueWinCount
            ...
            self.tableLayer:updateLeftCardCount(80 - GameCommon.gameConfig.bPlayerCount*14 - 1, true)
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_TI_CARD then
            ...
            for i = 1 ,  GameCommon.gameConfig.bPlayerCount do
                local ChairID = i - 1                  
                self:showMoneyView(ChairID,pBuffer.lUserScore[i])
                GameCommon.player[ChairID].lScore=GameCommon.player[ChairID].lScore+pBuffer.lUserScore[i]
            end
            self:updatePlayerInfo()
        elseif subCmdID == NetMsgId.SUB_S_USER_PAO_CARD then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.paohuzi.16.GameEndLayer"):create(pBuffer))
                    end
                    end),
            ...
            for i = 1 ,  GameCommon.gameConfig.bPlayerCount do
                local wViewChairID =  i - 1
                self:showMoneyView(wViewChairID,pBuffer.lGameScore[i])
        ...
        elseif subCmdID == NetMsgId.SUB_S_ADD_BASE then    --用户加倍
            self.tableLayer:doAction(GameCommon.ACTION_WUFU_ADD_BASE,pBuffer)
            
        elseif subCmdID == NetMsgId.SUB_S_ADD_BASE_VIEW then    --用户加倍表现
            local wViewChairID= GameCommon:getViewIDByChairID(pBuffer.wActionUser)
            GameCommon.wufu[wViewChairID] = nil 
            GameCommon.wufu[wViewChairID] = pBuffer.IsAdd
            self:updatePlayerInfo()
            self.tableLayer:doAction(GameCommon.ACTION_WUFU_ADD_BASE_VIEW,pBuffer)
...
function GameLayer:showMoneyView(wChairID,money)--显示输赢
    if money == 0 then
        return
    end
    local viewID = GameCommon:getViewIDByChairID(wChairID)  --视图     
    print("玩家视图位置：",wChairID,viewID,money)
    local uiPanel_tipsCardPos = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_userDefen%d",viewID)) 
    local pt = cc.p(uiPanel_tipsCardPos:getPosition())    
    local diceCount = nil
    if money > 0 then
        diceCount= ccui.TextAtlas:create(string.format(":%d",money),"fonts/fonts_6.png",26,43,'0')
    else
        diceCount= ccui.TextAtlas:create(string.format(":%d",money),"fonts/fonts_7.png",26,43,'0')
    end
    diceCount:setAnchorPoint(cc.p(0.5,0.5))
    diceCount:setPosition(pt)
    diceCount:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),cc.ScaleTo:create(0.5,1.2),cc.ScaleTo:create(0.5,1.0),cc.FadeOut:create(0.5),cc.RemoveSelf:create(true)))
    self:addChild(diceCount,2)
end
...
function GameLayer:updatePlayerInfo()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.tableConfig.nTableType ~= TableType_GoldRoom then
            if GameCommon.wContinueWinCount ~= nil and GameCommon.wContinueWinCount > 0 and i == GameCommon.wBankerUser + 1 then
                local node = uiImage_other:getParent():getChildByTag(1000)
                if node then
                    node:removeFromParent()
                end
                local lianzhuang = ccui.ImageView:create("zipai/table/lianzhuang.png")
                lianzhuang:removeAllChildren()
                uiImage_other:getParent():addChild(lianzhuang)             
                local bei = ccui.TextAtlas:create(string.format("%d",GameCommon.wContinueWinCount),"fonts/fonts_11.png",20,29, '0')    
                ...
                if viewID >= 3 then
                    ...
                    lianzhuang:setPosition(cc.p(- bei:getContentSize().width + 105,lianzhuang:getParent():getContentSize().width*0.7 + 20))
                    bei:setAnchorPoint(cc.p(0,0.5))
                    bei:setPosition(cc.p(bei:getParent():getContentSize().width - 70,bei:getParent():getContentSize().height/2))
                else
                    lianzhuang:setAnchorPoint(cc.p(0,0.5))
                    lianzhuang:setPosition(cc.p(bei:getParent():getContentSize().width + 50,lianzhuang:getParent():getContentSize().width*0.7 + 20))
                    bei:setAnchorPoint(cc.p(0,0.5))
                    bei:setPosition(cc.p(bei:getParent():getContentSize().width - 70,bei:getParent():getContentSize().height/2))
                end
            else
        ...
        --五福报警   
        --print("五福报警", self.wufu[viewID])
        if  GameCommon.wufu[viewID]~=nil then            
            if GameCommon.wufu[viewID] == true  then-- GameCommon.wufu[viewID] == viewID and .add
                ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("zipai/animation/wufubaojing-texiao2/wufubaojing-texiao.ExportJson")
                local armature = ccs.Armature:create("wufubaojing-texiao")
                armature:setScale(0.3)
                armature:getAnimation():playWithIndex(0)
                uiImage_other:addChild(armature)
                armature:setTag(1001)
                    if viewID >= 3 then
                        armature:setPosition(cc.p(-armature:getParent():getContentSize().width+105,armature:getParent():getContentSize().height/2+40))
                    else
                        armature:setPosition(cc.p(armature:getParent():getContentSize().width-65,armature:getParent():getContentSize().height/2+40))
                    end         
            end
        end 
    end       
...
function GameLayer:updatePlayerlScore()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
        local uiText_score = ccui.Helper:seekWidgetByName(uiPanel_player,"Text_score")
        print ("GameCommon:",GameCommon.player[i-1].lScore)
        local dwGold = Common:itemNumberToString(GameCommon.player[wChairID].lScore)
        uiText_score:setString(tostring(dwGold))   
    end
end
...
function GameLayer:updateBankerUser()
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player[wChairID] ~= nil and GameCommon.player[wChairID].wChairID == GameCommon.wBankerUser then
            ...
            print("装：",GameCommon.wContinueWinCount)
            if GameCommon.wContinueWinCount == 0 then
