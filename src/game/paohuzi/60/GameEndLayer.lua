...
function GameEndLayer:showMingTang(pBuffer)
    ...
    self.PHZ_HT_DUIDUIHU = 64           --对对胡   全部对子                   								(X 4倍)																						(X 4倍)
    self.PHZ_HT_HONGDUI = 128 		    --红对(6) 											(X 6倍)
    self.PHZ_HT_WUDUI = 256		        --乌对(8) 											(X 8倍)
    self.PHZ_HT_JIAHONGDUI = 512        --夹红对(8) 											(X 8倍)
    self.PHZ_HT_TIANHU = 1024		    --                                                                                              (X 4倍)
    ...
    --胡牌类型大字胡，小胡子，红乌需要计算基数
        --红对胡
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HONGDUI) ~= 0 then
        local item = uiPanel_defaultPalyer:clone() 
        self:createMingTang(item,"hongduihu",6 ,"","fan")
        uiListView_player:pushBackCustomItem(item)
    elseif Bit:_and(pBuffer.wType,self.PHZ_HT_HONGHU) ~= 0 then    --红胡
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"honghu",2 ,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
    
    if Bit:_and(pBuffer.wType,self.PHZ_HT_JIAHONGDUI) ~= 0 then
        local item = uiPanel_defaultPalyer:clone() 
        self:createMingTang(item,"jiahongdui",8 ,"","fan")
        uiListView_player:pushBackCustomItem(item)
    elseif Bit:_and(pBuffer.wType,self.PHZ_HT_HONGWU) ~= 0 then    --红乌
        local item = uiPanel_defaultPalyer:clone()
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_WUDUI) ~= 0 then
    	local item = uiPanel_defaultPalyer:clone() 
    	self:createMingTang(item,"wuduihu",8,"","fan")
    	uiListView_player:pushBackCustomItem(item)
     elseif Bit:_and(pBuffer.wType,self.PHZ_HT_HEIWU) ~= 0 then    --黑胡
        local item = uiPanel_defaultPalyer:clone()
	    self:createMingTang(item,"heihu",4,"","fan")
        uiListView_player:pushBackCustomItem(item)
     end
