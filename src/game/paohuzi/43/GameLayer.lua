...
local TableLayer = require("game.paohuzi.43.TableLayer")
...
function GameLayer:startGame(...)
    ...
    local csb = cc.CSLoader:createNode("GameXuPuPaoHuZiLayer.csb")
    self:addChild(csb)
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_CHONG_FEN then    
            _tagMsg.pBuffer.nChongFen = {}                         --吃牌组合
            for i = 1 , 3 do
                _tagMsg.pBuffer.nChongFen[i] = luaFunc:readRecvInt()
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_WEI_CARD then
            _tagMsg.pBuffer.wActionUser = luaFunc:readRecvWORD()    --动作用户
            _tagMsg.pBuffer.cbActionCard = {}
            for i = 1, 4 do
                _tagMsg.pBuffer.cbActionCard[i] = luaFunc:readRecvByte()   --操作扑克
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_SISHOU then
            _tagMsg.pBuffer.bUserSiShou = {}
            for i = 1, 3 do
                _tagMsg.pBuffer.bUserSiShou[i] = luaFunc:readRecvByte()
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            _tagMsg.pBuffer.HuCardInfo.cbCardEye = {}
            for i = 1, 2 do
                _tagMsg.pBuffer.HuCardInfo.cbCardEye[i] = luaFunc:readRecvByte()--牌眼扑克
            end
...
--            local skinIndex = cc.UserDefault:getInstance():getIntegerForKey(Default.UserDefault_Skin,1)
--            local uiImage_gameBg = ccui.ImageView:create(string.format("game/paohuzi/res/table/word_plate_paizhuo%d.png",skinIndex))
--            self:addChild(uiImage_gameBg)
--            uiImage_gameBg:setPosition(cc.Director:getInstance():getVisibleSize().width/2,cc.Director:getInstance():getVisibleSize().height/2)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_CHONG_FEN then
            self.tableLayer:updateGameState(GameCommon.GameState_Start)
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                GameCommon.player[i-1].nChongFen = pBuffer.nChongFen[i]
            end
            self:updatePlayerChongFen()
            return
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            self.tableLayer:updateLeftCardCount(19, true)
            self:updateBankerUser()
            self:updatePlayerInfo()
            self:updatehandplate()
            self:updatePlayerSiShou()
            self:updatePlayerChongFen()
            self.tableLayer:showCountDown(pBuffer.wCurrentUser)
--            if GameCommon.tableConfig.nTableType ~= TableType_Playback then
--                if self.tableCardAll == nil then
--                    self.tableCardAll = {}
--                end
--                for i = 1, GameCommon.player[wChairID].bUserCardCount do
--                    table.insert(self.tableCardAll,#self.tableCardAll+1,pBuffer.cbCardData[i])
--                end
--                if wChairID == GameCommon.gameConfig.bPlayerCount-1 then
--                    local tempCard = self.tableCardAll[21]
--                    table.remove(self.tableCardAll,21)
--                    table.insert(self.tableCardAll,#self.tableCardAll+1,tempCard)
--                end
--            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_USER_CHI_CARD then
            ...
            self:updatePlayerHuXi(pBuffer.wActionUser) 
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY then
            if GameCommon.tableConfig.nTableType == TableType_GoldRoom and os.time() - GameCommon.hostedTime > 60 then
                local uiPanel_hosted = ccui.Helper:seekWidgetByName(self.root,"Panel_hosted")
                uiPanel_hosted:setVisible(true)
                --NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_OPERATE_CARD,"bwwbb",GameCommon.CK_NULL,0,GameCommon.ACK_NULL,0,0,0)
                return
            else
        ...
        elseif subCmdID == NetMsgId.SUB_S_OUT_CARD_NOTIFY then
            if GameCommon.tableConfig.nTableType == TableType_GoldRoom and os.time() - GameCommon.hostedTime > 60 and pBuffer.wCurrentUser == GameCommon:getRoleChairID() then
                local uiPanel_hosted = ccui.Helper:seekWidgetByName(self.root,"Panel_hosted")
                uiPanel_hosted:setVisible(true)
--                local wChairID = GameCommon:getRoleChairID()
--                for i=1 , 40 do
--                    if (GameCommon.player[wChairID].cbCardIndex[i]==1) then
--                        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_OUT_CARD,"b",GameLogic:SwitchToCardData(i))
--                        break
--                    end
--                end
--                for i=1 , 40 do
--                    if (GameCommon.player[wChairID].cbCardIndex[i]==2) then
--                        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_OUT_CARD,"b",GameLogic:SwitchToCardData(i))
--                        break
--                    end
--                end
...
--            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_SISHOU then
            for i = 0, GameCommon.gameConfig.bPlayerCount-1 do
                if GameCommon.player[i] ~= nil then
                    GameCommon.player[i].bSiShou = pBuffer.bUserSiShou[i+1]
                end
            end
            self:updatePlayerSiShou()
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
...
--            if GameCommon.tableConfig.nTableType ~= TableType_Playback then
--                local str = "[phz]\n"
--                for key, var in pairs(self.tableCardAll) do
--                    if key%20 == 0 then
--                        str = str..string.format("CARD_%d = %#x\n\n",80-key,tostring(var))
--                    else
--                        str = str..string.format("CARD_%d = %#x\n",80-key,tostring(var))
--                    end
--                end
--                for key = #self.tableCardAll+1, 80 do
--                    str = str..string.format("CARD_%d = %#x\n",80-key,tostring(1))
--                end
--                local fp = io.open(FileName.phzPAI,"wb+")
--                fp:write(str)
--                fp:close()
--            end
            ...
            self.tableLayer:doAction(GameCommon.ACTION_HU_CARD, {cbReason = pBuffer.cbReason, cbHuCard = pBuffer.cbHuCard, wWinUser = pBuffer.wWinUser, wProvideUser = pBuffer.wProvideUser,wType = pBuffer.wType})
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.paohuzi.43.GameEndLayer"):create(pBuffer))
                    end
                    end),
...
--                                GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            --游戏重连
            self.tableLayer:updateGameState(GameCommon.GameState_Start)
            local wChairID = GameCommon:GetMeChairID()
            GameCommon.wBankerUser = pBuffer.wBankerUser
            local isSwitchChongFen = true
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                GameCommon.player[i-1].bSiShou = pBuffer.bUserSiShou[i]
                GameCommon.player[i-1].nChongFen = pBuffer.nChongFen[i]
                if GameCommon.player[i-1].nChongFen == -1 then
                    isSwitchChongFen = false
                end
...
function GameLayer:EVENT_TYPE_OPERATIONAL_OUT_CARD(event)
    ...
    local targetValue = Bit:_and(cbCardData,0x0F)
    local targetColor = Bit:_and(cbCardData,0xF0)
    for i = 1, GameCommon.gameConfig.bPlayerCount do
        if i-1 ~= GameCommon:getRoleChairID() then
            for j = 1, GameCommon.player[i-1].bWeaveItemCount do
                if GameCommon.player[i-1].WeaveItemArray[j].cbWeaveKind == GameCommon.ACK_WEI then
                    local cbCenterCard = GameCommon.player[i-1].WeaveItemArray[j].cbCenterCard
                    local value = Bit:_and(cbCenterCard,0x0F)
                    local color = Bit:_and(cbCenterCard,0xF0)
                    if value == targetValue and ((targetColor <= 0x10 and color <= 0x10) or (targetColor > 0x10 and color > 0x10)) then
                        require("common.MsgBoxLayer"):create(1,nil,"放跑之后,不能变动手牌?",function(sender,event) 
...
function GameLayer:updatehandplate()
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        uiText_Houdplate:setString(string.format("%d张",GameCommon.player[wChairID].bUserCardCount))
    end
end
...
function GameLayer:updatePlayerHuXi(wChairID)
    ...
    if viewID == 1 then --+GameCommon.handHuXiNum
        uiText_huXi:setString(string.format("%d胡息",huXiCount))
    end 
end
...
function GameLayer:updatePlayerSiShou()
    if GameCommon.gameConfig == nil then
        return
    end
    local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,"Panel_player")
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil then
            ...
            if GameCommon.player[wChairID].bSiShou == 1 then            
                local logo = string.format("tszipai/table/laopai_table11.png")   
                ...
                uiImage_other:setVisible(true)
            elseif GameCommon.player[wChairID].bSiShou == 2 then         
    ...
    if GameCommon.player[wChairID].bSiShou == 0 then
...
function GameLayer:updatePlayerChongFen()
    if GameCommon.gameConfig == nil then
        return
    end
    local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,"Panel_player")
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil then
            local viewID = GameCommon:getViewIDByChairID(wChairID)
            local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
            local uiImage_chongFen = ccui.Helper:seekWidgetByName(uiPanel_player,"Image_chongFen")
            uiImage_chongFen:setVisible(true)
            if GameCommon.player[wChairID].nChongFen == 0 then
                uiImage_chongFen:loadTexture("tszipai/table/laopai_table8.png")
            elseif GameCommon.player[wChairID].nChongFen == 1 then
                uiImage_chongFen:loadTexture("tszipai/table/laopai_table5.png")
            elseif GameCommon.player[wChairID].nChongFen == 2 then
                uiImage_chongFen:loadTexture("tszipai/table/laopai_table6.png")
            elseif GameCommon.player[wChairID].nChongFen == 3 then
                uiImage_chongFen:loadTexture("tszipai/table/laopai_table7.png")
            else
    ...
    if GameCommon.player[wChairID].nChongFen == -1 and GameCommon.gameConfig.bChongFen == 1 then
        uiPanel_chongFen:setVisible(true)
    else
