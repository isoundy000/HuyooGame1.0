...
function GameEndLayer:showMingTang(pBuffer)
    ...
    self.XiaoKaHu                   = 0x800000              --小卡胡
    self.DaKaHu                     = 0x1000000             --大卡胡
    self.WuHu                       = 0x2000000             --无胡

--   点胡：胡牌时，玩家有且仅有1只红字。胡息×2
--红胡：胡牌时，玩家牌中有13个红的或以上，胡息×2
--黑胡：胡牌时，玩家没有红字。番数：×2
--自摸：胡牌时，所胡的那只牌是玩家自己从墩上
--摸上来的，番数：2番
--天胡：庄家胡亮张，胡息×2
--地胡（举手）只要没有动牌胡牌就算地胡，胡息×2
--无胡：胡牌的时候没有一胡牌，算21胡
--小卡胡：胡牌的时候是10胡，就算16胡
--大卡胡：胡牌的时候是20胡，就算24胡
--放炮：可以胡打出来的牌，放炮者出两家的钱
--提龙：小字：1分   大字：2分，只要倾牌了直接算钱
--囤数计算：从10胡息开始计算，11-15胡息为1囤，16-19胡
--息为2囤，21-23为3囤，24-26为4囤，以后每增加3胡息增加一囤
--以此类推（注：因为10胡和20胡属于卡胡）
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HongHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"honghu",2,".","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HeiHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"heihu",2,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
    --点胡判断
    if Bit:_and(pBuffer.wType,self.PHZ_HT_DianHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"dianhu",2,".","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    --小卡胡
    if Bit:_and(pBuffer.wType,self.XiaoKaHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"xiaokahu",16,"=","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    --大卡胡
    if Bit:_and(pBuffer.wType,self.DaKaHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"dakahu",24,"=","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    --无胡
    if Bit:_and(pBuffer.wType,self.WuHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"wuhu",21,"=","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    --地胡
    if Bit:_and(pBuffer.wType,self.PHZ_HT_DiHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"dihu",2,".","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    --听胡
    if Bit:_and(pBuffer.wType,self.PHZ_HT_TingHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"tinghu",2,".","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    --天胡
    if Bit:_and(pBuffer.wType,self.PHZ_HT_TianHu)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"tianhu",2,".","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
...
function GameEndLayer:createMingTang(item,mingTang,num,numType,unit)
    ...
    if unit ~= "" then
        ...
        uiImage_type:setPosition(cc.p(uiImage_type:getParent():getContentSize().width*0.585,uiImage_type:getParent():getContentSize().height/2))
    else
...
function GameEndLayer:getSptWeaveType(type)
    ...
    elseif type == GameCommon.ACK_WEI then
        sptname="zipai/table/endlayer29.png"
