...
local GameOperation = require("game.paohuzi.61.GameOperation")
...
function TableLayer:doAction(action,pBuffer)
    ...
    if action == GameCommon.ACTION_OUT_CARD_NOTIFY then
        ...
        if pBuffer.bCanLiu == 1 and pBuffer.wCurrentUser == GameCommon:getRoleChairID()  then 
            ...
            local oprationLayer = GameOperation:create(3)--,pBuffer.cbOperateCode,pBuffer.cbActionCard,GameCommon.player[wChairID].cbCardIndex,pBuffer.cbSubOperateCode)
    ...
    elseif action == GameCommon.ACTION_OPERATE_NOTIFY then    -- cbActionCard
...
function TableLayer:setChidata(wChairID)  --添加不能吃的牌
    local  bWeaveItemCount = GameCommon.player[wChairID].bWeaveItemCount
    local  WeaveItemArray = GameCommon.player[wChairID].WeaveItemArray
    ...
    for key = 1, bWeaveItemCount do
       ...
       -- for k, v in pairs(var.cbCardList) do
            if GameLogic:IsValidCard(var.cbCardList[1]) then
                local card = nil
                if var.cbWeaveKind == GameCommon.ACK_CHI then
                    -- local  v[k]  
                   local ChiType =  GameLogic:GetChiType(var)
                   table.insert(Cannotcard,#Cannotcard + 1,var.cbCenterCard)

                   local cbCurrentIndex= GameLogic:SwitchToCardIndex(var.cbCenterCard)   

                    if cbCurrentIndex > 10 then
                        cbCurrentIndex=cbCurrentIndex - 10
                    end
                    if ChiType ==  GameCommon.CK_LEFT then 
                            if cbCurrentIndex <= 7 then 
                                table.insert(Cannotcard,#Cannotcard + 1,var.cbCenterCard+3)
                            end 
                    elseif ChiType ==  GameCommon.CK_LEFT then 

                    elseif ChiType ==  GameCommon.CK_RIGHT then 
                            if cbCurrentIndex >= 4 then 
                                table.insert(Cannotcard,#Cannotcard + 1,var.cbCenterCard-3)
                            end 
                    end 
                elseif var.cbWeaveKind == GameCommon.ACK_WEI then 
                ...
                elseif var.cbWeaveKind == GameCommon.ACK_PENG then 
                    table.insert(Cannotcard,#Cannotcard + 1,var.cbCenterCard)
                end 
            end 
       -- end 
    ...
    if Cannotcard ~= {} then 
        GameCommon.Cannotcard = Cannotcard
    end 
end 
...
function TableLayer:setHandCard(wChairID,bUserCardCount,cbCardIndex,maxHanCardRow,cbCardCoutWW, isReconnect)
    ...
    local visibleSize = cc.Director:getInstance():getVisibleSize()
    local uiPanel_stacks = ccui.Helper:seekWidgetByName(self.root,"Panel_stacks")
    GameCommon.player[wChairID].cardStackInfo = GameLogic:sortHandCard(clone(cbCardIndex), maxHanCardRow, cc.p(uiPanel_stacks:getPosition()), 3)
...
function TableLayer:showHandCard(wChairID,effectsType,isShowEndCard)
    ...
    if isShowEndCard == true then
        ...
        if viewID == 1 and GameCommon.gameConfig.bPlayerCount == 4 and StaticData.Games[GameCommon.tableConfig.wKindID].isZuoXing4 == 0  then-- and (CHANNEL_ID == 0 or CHANNEL_ID == 1 or CHANNEL_ID == 10 or CHANNEL_ID == 11 or GameCommon:judgeGame())
            cardScale = 0.6
        end
    else
        local uiPanel_card = ccui.Helper:seekWidgetByName(self.root,"Panel_card")
...
--                card:runAction(cc.Sequence:create(cc.DelayTime:create(1*index*0.03),cc.Spawn:create(cc.ScaleTo:create(time,cardScale),cc.MoveTo:create(time,v.pt))))
            ...
            elseif effectsType == 2 then
                ...
                --听牌角标
                if GameCommon.Cannotcard[1] ~= nil then 
                    local ting = true
                    if  v.data ==  26  then 
                        local a =1
                    end 
                    for k_a, v_a in pairs(GameCommon.Cannotcard) do
                        if v.data ~= GameCommon.CardData_WW and v.data == v_a then  
                            ting = false
                        end
                    end
                    if ting == true  then
            ...
            if isCanMove == true then --主角位置才能拖动手牌               
                for key, var in pairs(GameCommon.Cannotcard) do
                    if v.data ~= GameCommon.CardData_WW and v.data == var then  
...
function TableLayer:initUI()
    ...
    for i = 1, 4 do
     ...
     local maipiao = {[1] = {1,2,3},[2] = {2,3,5}}

    local uiPanel_maipiao = ccui.Helper:seekWidgetByName(self.root,"Panel_maipiao")
    -- uiPanel_maipiao:setVisible(false)

    local uiButton_piaofen1 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen1")        --飘一分 
    local uiButton_piaofen2 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen2")        --飘两分
    local uiButton_piaofen3 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen3")        --飘三分
   
    local uiButton_piaofen0 = ccui.Helper:seekWidgetByName(self.root,"Button_piaofen0")        --不飘
    local number = 1 
    if GameCommon.tableConfig.tableParameter ~= nil and GameCommon.tableConfig.tableParameter.bPiaoFen ~= nil and GameCommon.tableConfig.tableParameter.bPiaoFen ~= 0 then
        number = GameCommon.tableConfig.tableParameter.bPiaoFen
    end  
    for i = 1 , 3 do
        local uiButton_piaofen = ccui.Helper:seekWidgetByName(self.root,string.format("Button_piaofen%d",i))        --飘一分    
        local textureName =string.format("game/pukenew_scorebtn_%d.png",maipiao[number][i])
        local texture = cc.TextureCache:getInstance():addImage(textureName)
        uiButton_piaofen.data = maipiao[number][i]
        uiButton_piaofen:loadTextures(textureName,textureName,textureName)
        uiButton_piaofen:setContentSize(texture:getContentSizeInPixels())  
        uiButton_piaofen:setPressedActionEnabled(true)
        Common:addTouchEventListener(uiButton_piaofen,function() 
            NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_USER_PIAO_FEN,"i",uiButton_piaofen.data)
        end)
    end 
    Common:addTouchEventListener(uiButton_piaofen0,function() 
        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_USER_PIAO_FEN,"i",-1)
    end)
   ...
   --  if  GameCommon:judgeGame() or  CHANNEL_ID == 0 or CHANNEL_ID == 1 or CHANNEL_ID == 10 or CHANNEL_ID == 11  then
    ...
    Common:addTouchEventListener(uiButton_return,function() 
        ...
        -- if randCeil == 0 and (GameCommon.meChairID ~= 0 or GameCommon.tableConfig.nTableType == TableType_ClubRoom ) and GameCommon.player[GameCommon.meChairID ].bReady ~= true and GameCommon:judgeGame() then       
        --     NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_USER,NetMsgId.REQ_GR_LEAVE_TABLE_USER,"")
        -- else
...
function TableLayer:playSketlAnim(sChairID, eChairID, index,indexEx)
	...
	local Animation = require("game.paohuzi.61.Animation")
