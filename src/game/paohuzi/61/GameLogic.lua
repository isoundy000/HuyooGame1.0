...
function GameLogic:GetActionChiCard(cbCardIndex, cbCurrentCard)--吃牌判断
    ...
    if bCardValue == 2 or bCardValue == 7 or bCardValue == 10 then
        ...
        for i = 1 , #cbExcursion do
            ...
            if (cbIndex ~= cbCurrentIndex) and (cbCardIndex[cbIndex] == 0)  then
                break
            end
            index = i
        end
    ...
    for i = 1 , #cbExcursion do
        ...
        if (cbValueIndex >= cbExcursion[i]) and ((cbValueIndex - cbExcursion[i]) <= 7) then
            ...
            if index == #cbExcursion then
                ...
                --GameCommon.bAbandonChiCard
                local addchi = true
                if i == 1  and (cbCurrentIndex <= 17 or(cbCurrentIndex<= 7 and cbCurrentIndex >= 1 )) then
                   if GameCommon.bAbandonChiCard[1]~=nil and GameCommon.bAbandonChiCard[cbCurrentIndex+3] ~= false then 
                        addchi = false
                   end 
                elseif i == 3 and (cbCurrentIndex >= 4 or(cbCurrentIndex<= 20 and cbCurrentIndex >= 14 ))then 
                    if GameCommon.bAbandonChiCard[1]~=nil and  GameCommon.bAbandonChiCard[cbCurrentIndex-3] ~= false then 
                        addchi = false
                    end 
                end 
...
-- 获取顺子吃牌类型
function GameLogic:GetChiType(var)    
    local cbExcursion = {[1]= 1,[2]= 2,[3]= 3}
    -- var.cbCardCount = luaFunc:readRecvByte()    --扑克数目    3
    -- var.cbCenterCard = luaFunc:readRecvByte()   --中心扑克     1 
    -- var.cbCardList = {}
    -- for o = 1 , 4 do
    --     var.cbCardList[o] = luaFunc:readRecvByte()  --扑克列表   1 2 3
    -- end
--   local cbCurrentIndex=self:SwitchToCardIndex(var.cbCenterCard)   
    local cbCardList = {}
    for i = 1 ,3 do 
        cbCardList[i] = var.cbCardList[i]
    end
    local a= 1
    for i = 1 , 2 do 
        for j = i+1, 3 do
            local a = 0
            if cbCardList[i] > cbCardList[j] then 
                a = cbCardList[j]
                cbCardList[j] = cbCardList[i]
                cbCardList[i] = a
            end 
        end 
    end 
    if(self:SwitchToCardIndex(cbCardList[1]) == 2 and  self:SwitchToCardIndex(cbCardList[2]) == 7  and  self:SwitchToCardIndex(cbCardList[3]) == 10 ) or
    (self:SwitchToCardIndex(cbCardList[1]) == 12 and  self:SwitchToCardIndex(cbCardList[2]) == 17  and  self:SwitchToCardIndex(cbCardList[3]) == 20 ) then 
        return 0
    end 
    local a= 1
    for i = 1, 3 do 
        if var.cbCenterCard == cbCardList[i]  then 
            local cbChiKind = {[1] = GameCommon.CK_LEFT,[2] = GameCommon.CK_CENTER,[3] = GameCommon.CK_RIGHT}
            return cbChiKind[i]
        end  
    end 
...
function GameLogic:GetHuCardInfoData(wChairID, cbCardIndex, cbCurrentCard)--胡牌结果
    ...
    if cbCardIndex[cbCurrentCard] == 2 then
        ...
        -- else
            bUserHuXiCountModify2 = bUserHuXiCountModify2 + 4
        -- end
        paotiwei = true
    elseif cbCardIndex[cbCurrentCard] == 3 then
        ...
        for i=1,WeaveItemCountModify do
            if WeaveItemArrayModify[i].cbWeaveKind == GameCommon.ACK_WEI and WeaveItemArrayModify[i].cbCenterCard == cbCenterCard then
                ...
                bUserHuXiCountModify2 = bUserHuXiCountModify2 + 1
                paotiwei = true
                break
            end
...
function GameLogic:GetWeaveHuXi(WeaveItem)--获取胡息
    ...
    elseif WeaveItem.cbWeaveKind == GameCommon.ACK_KAN then  --坎
    -- if Bit:_and(WeaveItem.cbCenterCard, GameCommon.MASK_COLOR)==16 then
    --     return 3
    -- else
        return 3
...
function GameLogic:sortHandCardthree(cardIndex, maxHanCardRow,pos)
    ...
    for i=1 , 20 do
        ...
        if #cardStackInfo < maxHanCardRow and i <= 9 then
            if cardIndex[i]==1  and  cardIndex[i+1]==1 then
                local cardinfo = {}
                cardinfo.nCardCount = 2
                cardinfo.cbCardData = {}
            ...
            if cardIndex[i]==1  and  cardIndex[i+2]==1 then
                local cardinfo = {}
                cardinfo.nCardCount = 2
                cardinfo.cbCardData = {}
                cardIndex[i]=0
        ...
        if #cardStackInfo < maxHanCardRow and i <= 19  and i > 10  then
            if cardIndex[i]==1  and  cardIndex[i+1]==1 then
                local cardinfo = {}
                cardinfo.nCardCount = 2
                cardinfo.cbCardData = {}
        ...
        if cardIndex[i]==1 then
            if #cardStackInfo < maxHanCardRow then
                ...
                for j= #cardStackInfo , 1 , -1 do
                    local num1 = cardStackInfo[j].cbCardData[1].data 
                    if cardStackInfo[j].nCardCount < 3 and ((num1 <= 10 and i <= 10) or (num1 > 10 and i > 10))then
                        cardStackInfo[j].nCardCount = cardStackInfo[j].nCardCount + 1
                        cardIndex[i]=0
                        local _cardData = {}
                        _cardData.data=GameLogic:SwitchToCardData(i)
...
function GameLogic:sortEndHandCard(cardIndex, maxHanCardRow)
    ...
    for i=1 , 20 do
        ...
        if cardIndex[i]==1 then
            if #cardStackInfo < maxHanCardRow and i > 10 then
                local cardinfo = {}
                cardinfo.nCardCount = 1
                cardinfo.cbCardData = {}
                for j= 1 , cardIndex[i] do 
...
function GameLogic:CalculateColHuXi(colData)
    ...
    if len <  2 then
        return 0
    elseif len == 2 then
        return self:_calculateTwoHuXi(colData) 
    elseif len == 3 then
...
function GameLogic:_calculateTwoHuXi(colData)
    ...
    if isCardEquil(colData) then
        ...
        if colData[1].data== 2  then
            if colData[2].data == 7 then 
                return 1
            elseif colData[2].data == 10 then 
                return 1
            end
        elseif colData[1].data == 7  then
            if colData[2].data == 2 then 
                return 1
            elseif colData[2].data == 10 then 
                return 1
            end
        elseif colData[1].data == 10  then
            if colData[2].data == 7 then 
                return 1
            elseif colData[2].data == 2 then 
                return 1
            end
        elseif colData[1].data == 18  then
            if colData[2].data == 23 then 
                return 1
            elseif colData[2].data == 26 then 
                return 1
            end
        elseif colData[1].data == 23 then
            if colData[2].data == 18 then 
                return 1
            elseif colData[2].data == 26 then 
                return 1
            end
        elseif colData[1].data == 26 then
            if colData[2].data == 18 then 
                return 1
            elseif colData[2].data == 23 then 
                return 1
            end
        end
        -- local a = {[1]={2,7,10},[2]={18,23,26}} 
        -- local b = a[1]
        -- if colData[1].data > 18 then
        --     b = a[2]
        -- end 
        -- for i=1,2 do   
        --     local number = 0       
        --     for j =1,3 do
        --         if colData[i].data == b[j] then
        --             number = number +1
        --         end 
        --     end
        --     if number == 2 then
...
function GameLogic:_calculateThreeHuXi(colData)
    ...
    if isCardEquil(colData) then
        ...
        --     return 3
        if colData[1].data== 2 and colData[2].data == 7 and colData[3].data == 10 then
            return 1
