...
local TableLayer = require("game.paohuzi.61.TableLayer")
...
function GameLayer:startGame(...)
    ...
    GameCommon.Cannotcard = {} --不能出的牌   

    GameCommon.bAbandonChiCard = {} -- 不能吃的组合
...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_OUT_CARD_NOTIFY then
            ...
            _tagMsg.pBuffer.wCurrentUser = luaFunc:readRecvWORD()  --当前用户
            _tagMsg.pBuffer.bCanLiu = luaFunc:readRecvByte()  --是否可溜 
        ...
        elseif subCmdID == NetMsgId.SUB_S_PIAO_FEN_NOTIFY then--漂分提示无参数，该消息标志开始选择票分了
            self:updatePlayerPiaoFen(-1)       
            
            self.tableLayer:showCountDown(GameCommon.meChairID)
            return
        elseif subCmdID == NetMsgId.SUB_S_USER_PIAO_FEN then -- //用户漂分  
            _tagMsg.pBuffer.nPiaoFen = {}   --//用户漂分状态  -1不飘 ，0 没票 或者其他分 
            for i = 1 , 4 do
                _tagMsg.pBuffer.nPiaoFen[i] = luaFunc:readRecvInt() 
            end
            _tagMsg.pBuffer.wCurrentUser = luaFunc:readRecvWORD()    --当前漂分用户
            local a = 1

        elseif subCmdID == NetMsgId.SUB_S_USER_ABANDON_CHI_CARD then 
            _tagMsg.pBuffer.bAbandonChiCard = {}   
            for i = 1 , 20 do   --bool
                _tagMsg.pBuffer.bAbandonChiCard[i] = luaFunc:readRecvBool() 
            end
            local a = 1  
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then
            ...
            _tagMsg.pBuffer.lHaoFen = {}
            for i = 1 , 20 do
                _tagMsg.pBuffer.lHaoFen[i] = luaFunc:readRecvLong()--游戏豪分
            end
            _tagMsg.pBuffer.lTotalPiaoFen = luaFunc:readRecvLong()--总飘分
            --剩余扑克
            ...
            _tagMsg.pBuffer.HuCardInfo.cbCardEye1 = luaFunc:readRecvByte()--牌眼扑克
            _tagMsg.pBuffer.HuCardInfo.cbCardEye2 = luaFunc:readRecvByte()--牌眼扑克
            _tagMsg.pBuffer.HuCardInfo.cbHuXiCount = luaFunc:readRecvWORD()--胡息数目
            ...
            _tagMsg.pBuffer.bWeaveItemCount = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.bWeaveItemCount[i] = luaFunc:readRecvByte() -- 组合数目
            end 
            _tagMsg.pBuffer.WeaveItemArray = {}
            for n = 1 , 4 do
                _tagMsg.pBuffer.WeaveItemArray[n] = {}
                for i = 1 , 7 do
                    _tagMsg.pBuffer.WeaveItemArray[n][i] = {}
                    _tagMsg.pBuffer.WeaveItemArray[n][i].cbWeaveKind = luaFunc:readRecvByte()--组合类型
                    _tagMsg.pBuffer.WeaveItemArray[n][i].cbCardCount = luaFunc:readRecvByte()--扑克数目
                    _tagMsg.pBuffer.WeaveItemArray[n][i].cbCenterCard = luaFunc:readRecvByte()--中心扑克
                    _tagMsg.pBuffer.WeaveItemArray[n][i].cbCardList = {}    --扑克列表
                    for j = 1 , 4 do
                        _tagMsg.pBuffer.WeaveItemArray[n][i].cbCardList[j] = luaFunc:readRecvByte()
                    end
                end
            end 
            _tagMsg.pBuffer.cbCardIndex = {}
            for i = 1 , 4 do
                _tagMsg.pBuffer.cbCardIndex[i] = {}
                for j = 1, 20 do
                    _tagMsg.pBuffer.cbCardIndex[i][j] = luaFunc:readRecvByte()  -- 用户扑克
                end
            end
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            for i = 0, 3 do
                if GameCommon.player[i] ~= nil then
                    if i == pBuffer.wBankerUser then
                        ...
                        GameCommon.player[i].bUserCardCount = 19
                    end
                end
            ...
            if GameCommon.gameConfig.bDeathCard == 1 and GameCommon.gameConfig.bPlayerCount == 2  then
                self.tableLayer:updateLeftCardCount(80-2*19-21, true)
            elseif GameCommon.gameConfig.bDeathCard == 2 and GameCommon.gameConfig.bPlayerCount == 2  then
                    self.tableLayer:updateLeftCardCount(80-2*19-11, true)
            elseif GameCommon.gameConfig.bDeathCard == 0 and GameCommon.gameConfig.bPlayerCount == 2  then
                    self.tableLayer:updateLeftCardCount(80-2*19-1, true)
            elseif  GameCommon.gameConfig.bPlayerCount == 3  then
                self.tableLayer:updateLeftCardCount(80-GameCommon.gameConfig.bPlayerCount*19-1, true)
            else
                self.tableLayer:updateLeftCardCount(80-3*19-1, true)    
            end
        
    


            for i = 1, GameCommon.gameConfig.bPlayerCount do--//用户漂分状态  -1不飘 ，0 没票 或者其他分   飘分清零
                GameCommon.player[i-1].nPiaoFen = 0
            end 
        ...
        elseif subCmdID == NetMsgId.SUB_S_CLIENTERROR then
            ...
            self.tableLayer:setHandCard(wChairID, GameCommon.player[wChairID].bUserCardCount,pBuffer.cbCardIndex,11,0)
            self.tableLayer:showHandCard(wChairID, 0)
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        
        elseif subCmdID == NetMsgId.SUB_S_USER_PIAO_FEN then                
            -- _tagMsg.pBuffer.wCurrentUser = luaFunc:readRecvWORD()    --当前漂分用户
            for i = 1, GameCommon.gameConfig.bPlayerCount do--//用户漂分状态  -1不飘 ，0 没票 或者其他分 
                GameCommon.player[i-1].nPiaoFen = pBuffer.nPiaoFen[i]
            end    
            self:updatePlayerPiaoFen(pBuffer.wCurrentUser)   
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))       
        elseif subCmdID == NetMsgId.SUB_S_USER_ABANDON_CHI_CARD then -- //用户漂分 

            GameCommon.bAbandonChiCard =pBuffer.bAbandonChiCard
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.paohuzi.61.GameNewEndLayer"):create(pBuffer))
                    end
                    end),
...
--                                GameCommon:ContinueGame(GameCommon.tableConfig.cbLevel)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            self.tableLayer:setChidata(wChairID)
            ...
            if GameCommon.gameConfig.bPlayerCount == 4 and duijia == wChairID then
                self.tableLayer:setHandCard(GameCommon.wBankerUser,pBuffer.bUserCardCount[GameCommon.wBankerUser+1],pBuffer.cbCardIndex,11,0,true)
                self.tableLayer:showHandCard(GameCommon.wBankerUser,0)
            else
                self.tableLayer:setHandCard(wChairID,pBuffer.bUserCardCount[wChairID+1],pBuffer.cbCardIndex,11,0,true)
                self.tableLayer:showHandCard(wChairID,0)
            end
            self:updateBankerUser()
            if pBuffer.bOutCard ~= 0 then
                self.tableLayer:doAction(GameCommon.ACTION_OUT_CARD_NOTIFY, {wCurrentUser = wChairID, bOutCard = nil,bCanLiu = pBuffer.bCanLiu})
...
function GameLayer:updatePlayerPiaoFen(data)       
    if GameCommon.gameConfig == nil or GameCommon.gameConfig.bPiaoFen == 0 then
        return
    end   
    local uiPanel_maipiao = ccui.Helper:seekWidgetByName(self.root,"Panel_maipiao")   
    if  data == -1 then 
        uiPanel_maipiao:setVisible(true) 
    elseif data == GameCommon.meChairID then 
        print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++死了吗")
    ...
    for i = 1 , GameCommon.gameConfig.bPlayerCount do
        ...
        if GameCommon.player ~= nil and GameCommon.player[wChairID] ~= nil  then
            ...
            if GameCommon.player[wChairID].nPiaoFen ~= 0  then 
                uiImage_piao:setVisible(true)
                if GameCommon.player[wChairID].nPiaoFen == -1 then
                    uiImage_piao:loadTexture("game/mj_piaofen_1.png")
                elseif GameCommon.player[wChairID].nPiaoFen == 1 then
                    uiImage_piao:loadTexture("game/pukenew_score_1.png")
                elseif GameCommon.player[wChairID].nPiaoFen == 2 then
                    uiImage_piao:loadTexture("game/pukenew_score_2.png")
                elseif GameCommon.player[wChairID].nPiaoFen == 3 then
                    uiImage_piao:loadTexture("game/pukenew_score_3.png")
                elseif GameCommon.player[wChairID].nPiaoFen == 4 then
                    uiImage_piao:loadTexture("game/pukenew_score_4.png")
                elseif GameCommon.player[wChairID].nPiaoFen == 5 then
