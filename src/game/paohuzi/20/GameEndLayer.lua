...
--    end
    ...
    local item = uiPanel_defaultInfo:clone()
    local uiImage_name = ccui.Helper:seekWidgetByName(item,"Image_name")
    uiImage_name = ccui.ImageView:create("zipai/table/endlayer19.png")
    item:addChild(uiImage_name)
    uiImage_name:setAnchorPoint(cc.p(0,0.5))
    uiImage_name:setPosition(0,uiImage_name:getParent():getContentSize().height/2)
    local uiAtlasLabel_num = ccui.TextAtlas:create(string.format("%d",pBuffer.cbHuXiCount),"fonts/fonts_8.png",18,27,'.')
    item:addChild(uiAtlasLabel_num)
    uiAtlasLabel_num:setAnchorPoint(cc.p(1,0.5))
    uiAtlasLabel_num:setPosition(uiAtlasLabel_num:getParent():getContentSize().width,uiAtlasLabel_num:getParent():getContentSize().height/2)
    uiListView_info:pushBackCustomItem(item)
    ...
    for key, var in pairs(GameCommon.player) do
        ...
        if GameCommon.tableConfig.nTableType  ~= TableType_GoldRoom then 
            if pBuffer.lGameScore[var.wChairID+1]  <= 0 then
                ...
                uiAtlasLabel_money:setString(string.format("%d胡息",pBuffer.lGameScore[var.wChairID+1] ))
            else
                uiAtlasLabel_money:setString(string.format("+%d胡息",pBuffer.lGameScore[var.wChairID+1] ))
            end
		else
...
function GameEndLayer:showMingTang(pBuffer)
    self.PHZ_HT_NO                           =0x0000              --NULL
    self.PHZ_HT_ZIMO                         =0x0001              --自　摸  胡息*2
    self.PHZ_HT_HONGHU                       =0x0002              --十红 (>=10 <13) 胡息*2
    self.PHZ_HT_HONGWU                       =0x0004              --红胡 (>=13) 胡息=100
    self.PHZ_HT_DIANHU                       =0x0008              --一点红   胡息*2
    self.PHZ_HT_YIKUAIBIAN                   =0x0010              --一块扁   胡息*2 -->只有红字一句话、一坎、一提
    self.PHZ_HT_HAIDI                        =0x0020              --海底    胡息*2
    self.PHZ_HT_HEIHU                        =0x0040              --黑胡  胡息=100
    self.PHZ_HT_TIANHU                       =0x0080              --天胡    胡息=100
    self.PHZ_HT_DIHU                         =0x0100              --地胡    胡息=100
    self.PHZ_HT_KAHU                         =0x0200              --卡胡    胡息=100 -->胡息正好30
    self.PHZ_HT_20HU                         =0x0400              --20胡   胡息*2 -->胡息正好20
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_ZIMO) ~= 0 then
        ...
        self:createMingTang(item,"zimo",1,"","fan")
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HONGHU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"honghu",1,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HONGWU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"hongwu",100,"","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_DIANHU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"dianhu",1,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
    
    --一块扁
    if Bit:_and(pBuffer.wType,self.PHZ_HT_YIKUAIBIAN) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"yikuaibian",1,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HAIDI) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"haidihu",1,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_HEIHU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"heihu",100,"","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_TIANHU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"tianhu",100,"","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    ...
    if Bit:_and(pBuffer.wType,self.PHZ_HT_DIHU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"dihu",100,"","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    
    --30卡胡
    if Bit:_and(pBuffer.wType,self.PHZ_HT_KAHU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"30kahu",100,"","huxi")
        uiListView_player:pushBackCustomItem(item)
    end
    
    --20卡胡
    if Bit:_and(pBuffer.wType,self.PHZ_HT_20HU) ~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"20kahu",1,"","fan")
        uiListView_player:pushBackCustomItem(item)
    end
