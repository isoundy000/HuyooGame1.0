...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    if mainCmdID == NetMsgId.MDM_GR_USER then   
       if subCmdID == NetMsgId.SUB_GR_USER_READY then
        ...
        elseif subCmdID == NetMsgId.SUB_GR_USER_STATISTICS then
            ...
            for i= 1, 3 do
                local totalScore = _tagMsg.pBuffer.tScoreInfo[i].totalScore
                if math.abs(totalScore)%10 ~= 0 then
                	if totalScore > 0 and math.abs(totalScore)%10 >= 5 then
                		totalScore = totalScore - math.abs(totalScore)%10 + 10
            		elseif totalScore > 0 and math.abs(totalScore)%10 < 5 then
            		    totalScore = totalScore - math.abs(totalScore)%10
        		    elseif totalScore < 0 and math.abs(totalScore)%10 >= 5 then
        		        totalScore = totalScore + math.abs(totalScore)%10 - 10
        		        elseif totalScore < 0 and math.abs(totalScore)%10 < 5 then
                        totalScore = totalScore + math.abs(totalScore)%10
                ...
                _tagMsg.pBuffer.tScoreInfo[i].totalHuXi = totalScore
            end
            _tagMsg.pBuffer.tScoreInfo[1].totalScore = _tagMsg.pBuffer.tScoreInfo[1].totalHuXi*2 - _tagMsg.pBuffer.tScoreInfo[2].totalHuXi - _tagMsg.pBuffer.tScoreInfo[3].totalHuXi
            _tagMsg.pBuffer.tScoreInfo[2].totalScore = _tagMsg.pBuffer.tScoreInfo[2].totalHuXi*2 - _tagMsg.pBuffer.tScoreInfo[1].totalHuXi - _tagMsg.pBuffer.tScoreInfo[3].totalHuXi
            _tagMsg.pBuffer.tScoreInfo[3].totalScore = _tagMsg.pBuffer.tScoreInfo[3].totalHuXi*2 - _tagMsg.pBuffer.tScoreInfo[1].totalHuXi - _tagMsg.pBuffer.tScoreInfo[2].totalHuXi            
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then    --游戏轮数结算
            _tagMsg.pBuffer.isGameOver = luaFunc:readRecvBool()--游戏是否结束
            ...
            _tagMsg.pBuffer.cbHuXiCount = luaFunc:readRecvLong()    --//胡息数目
        ...
        elseif subCmdID == NetMsgId.SUB_S_QIPAI_NOTIFY then
            _tagMsg.pBuffer.wCurrentUser = luaFunc:readRecvWORD()     --当前用户
            _tagMsg.pBuffer.bIsQiPai = luaFunc:readRecvBool()         --是否弃牌
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
            ...
            _tagMsg.pBuffer.bQiPai = {}                                  --弃牌标志
            for i = 1 , 3 do
                _tagMsg.pBuffer.bQiPai[i] = luaFunc:readRecvBool()
            end
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_ADD_BASE then    --用户加倍
            self.tableLayer:doAction(GameCommon.ACTION_DATUO_ADD_BASE,pBuffer)
            
        elseif subCmdID == NetMsgId.SUB_S_ADD_BASE_VIEW then    --用户加倍表现
            local wChairID = pBuffer.wActionUser
            GameCommon.player[wChairID].other = pBuffer.IsAdd
            self:updatePlayerAdd(wChairID)
            self.tableLayer:doAction(GameCommon.ACTION_DATUO_ADD_BASE_VIEW,pBuffer)
            
        elseif subCmdID == NetMsgId.SUB_S_GAME_END then    --游戏结束  
                ...
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    cc.CallFunc:create(function(sender,event) 
                        if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                            ...
                            uiPanel_end:addChild(require("game.paohuzi.20.GameEndLayer"):create(pBuffer))
                        end
                    end),
                    cc.DelayTime:create(20),
                    cc.CallFunc:create(function(sender,event) 
                        if GameCommon.tableConfig.nTableType > TableType_GoldRoom then
                        if pBuffer.isGameOver == true then
        ...
        elseif subCmdID == NetMsgId.SUB_S_QIPAI_NOTIFY then
            self.tableLayer:updateDiscardState(pBuffer)
            self:runAction(cc.Sequence:create(cc.DelayTime:create(0),cc.CallFunc:create(function(sender,event) EventMgr:dispatch(EventType.EVENT_TYPE_CACEL_MESSAGE_BLOCK) end)))
        else 
            return print("error, not found this :",mainCmdID, subCmdID)
        end
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            --弃牌标志
            for i,v in ipairs(pBuffer.bQiPai) do
                local qpBuffer = {}
                qpBuffer.wCurrentUser = i - 1
                qpBuffer.bIsQiPai = v
                self.tableLayer:updateDiscardState(qpBuffer)
            end
...
function GameLayer:updatePlayerAdd(wChairID, effects)
    local huXiCount = 0
    local viewID = GameCommon:getViewIDByChairID(wChairID)
    local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
    ...
    local img = nil
    if GameCommon.player[wChairID].other == true then
        img = ccui.ImageView:create("zipai/table/datuo.png")
        uiImage_other:addChild(img)
    end
end
