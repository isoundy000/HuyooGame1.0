...
function GameEndLayer:showMingTang(pBuffer)
    ...
    self.PHZ_HT_ZIMO = 1              --自　摸
    self.PHZ_HT_HAIDI  = 2              --海底胡
    self.PHZ_HT_TIANHU =  4              --天胡    
    self.PHZ_HT_DIHU  = 8              --地胡
    ...
    --翻省
    if(pBuffer.fanXing[1].cbShengCout>=4) then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"fanxing",4,"+","")
        local _spt=GameCommon:getDiscardCardAndWeaveItemArray(pBuffer.fanXing[1].cbShengCard)
        _spt:setPosition(cc.p(120,22))
        item:addChild(_spt)
        uiListView_player:pushBackCustomItem(item)
        
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"fanxing",pBuffer.fanXing[1].cbShengCout-4,"+","")
        local _spt=GameCommon:getDiscardCardAndWeaveItemArray(pBuffer.fanXing[2].cbShengCard)
        _spt:setPosition(cc.p(120,22))
        item:addChild(_spt)
        uiListView_player:pushBackCustomItem(item)
        
    elseif(pBuffer.fanXing[1].cbShengCout>0 or pBuffer.fanXing[1].cbShengCard>0) then
        local item = uiPanel_defaultPalyer:clone()
        self:createMingTang(item,"fanxing",pBuffer.fanXing[1].cbShengCout,"+","")
        local _spt=GameCommon:getDiscardCardAndWeaveItemArray(pBuffer.fanXing[1].cbShengCard)
        _spt:setPosition(cc.p(120,22))
        item:addChild(_spt)
        uiListView_player:pushBackCustomItem(item)
...
function GameEndLayer:getSptWeaveType(type)
    ...
    if type == GameCommon.ACK_TI then
        sptname="zipai/table/endlayer6.png"
    elseif type == GameCommon.ACK_PAO then
        sptname="zipai/table/endlayer13.png"
    elseif type == GameCommon.ACK_WEI then
        sptname="zipai/table/endlayer5.png"
