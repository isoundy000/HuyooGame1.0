        ...
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            ...
            if GameCommon.gameConfig.bPlayerCount ~= 4 then
                if GameCommon.gameConfig.bDeathCard == 1 and GameCommon.gameConfig.bPlayerCount == 2  then
                    self.tableLayer:updateLeftCardCount(60 - GameCommon.gameConfig.bPlayerCount*20 - 1, true)
                else
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    if GameCommon.tableConfig.nTableType == TableType_SportsRoom then
                        ...
                        uiPanel_end:addChild(require("game.paohuzi.49.GameEndLayer"):create(pBuffer))
                    end
                    end),
...
function GameLayer:updatePlayerInfo()
    ...
    if CHANNEL_ID == 0 or CHANNEL_ID == 1 or CHANNEL_ID == 10 or CHANNEL_ID == 11 then 
        local uiImage_huangfan = ccui.Helper:seekWidgetByName(self.root,"Image_huangfan")
        if GameCommon.bIsHuangZhuang == true and GameCommon.tableConfig.nTableType ~= TableType_GoldRoom then
            local logo = ("zipai/table/end_play_huangfan.png")
            local texture = cc.TextureCache:getInstance():addImage(logo)        
            uiImage_huangfan:loadTexture(logo)
            uiImage_huangfan:setContentSize(texture:getContentSizeInPixels())
        else
            local logo = ("common/hall_5.png")
            local texture = cc.TextureCache:getInstance():addImage(logo)        
            uiImage_huangfan:loadTexture(logo)
